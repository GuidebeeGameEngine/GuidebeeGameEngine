...
extern "C" {
#endif
static uint32_t native_blend = BLEND_NONE;
static uint32_t native_scale = SCALE_NEAREST;
...
static inline set_pixel_func set_pixel_func_ptr(uint32_t format) {
	switch(format) {
		case FORMAT_ALPHA:			return &set_pixel_alpha;
		case FORMAT_LUMINANCE_ALPHA:	return &set_pixel_luminance_alpha;
		case FORMAT_RGB888:			return &set_pixel_RGB888;
		case FORMAT_RGBA8888:			return &set_pixel_RGBA8888;
		case FORMAT_RGB565:			return &set_pixel_RGB565;
		case FORMAT_RGBA4444:			return &set_pixel_RGBA4444;
...
static inline get_pixel_func get_pixel_func_ptr(uint32_t format) {
	switch(format) {
		case FORMAT_ALPHA:			return &get_pixel_alpha;
		case FORMAT_LUMINANCE_ALPHA:	return &get_pixel_luminance_alpha;
		case FORMAT_RGB888:			return &get_pixel_RGB888;
		case FORMAT_RGBA8888:			return &get_pixel_RGBA8888;
		case FORMAT_RGB565:			return &get_pixel_RGB565;
		case FORMAT_RGBA4444:			return &get_pixel_RGBA4444;
		default: return &get_pixel_alpha; // better idea for a default?
	}
}

native_pixmap* native_load(const unsigned char *buffer, uint32_t len) {
	...
	native_pixmap* pixmap = (native_pixmap*)malloc(sizeof(native_pixmap));
	if (!pixmap) return 0;
...
uint32_t native_bytes_per_pixel(uint32_t format) {
	switch(format) {
		case FORMAT_ALPHA:
			return 1;
		case FORMAT_LUMINANCE_ALPHA:
...
native_pixmap* native_new(uint32_t width, uint32_t height, uint32_t format) {
	native_pixmap* pixmap = (native_pixmap*)malloc(sizeof(native_pixmap));
	if (!pixmap) return 0;
	pixmap->width = width;
	pixmap->height = height;
	pixmap->format = format;
	pixmap->pixels = (unsigned char*)malloc(width * height * native_bytes_per_pixel(format));
	if (!pixmap->pixels) {
		free((void*)pixmap);
		return 0;
	}
...
void native_free(const native_pixmap* pixmap) {
	free((void*)pixmap->pixels);
	free((void*)pixmap);
}

void native_set_blend (uint32_t blend) {
	native_blend = blend;
}

void native_set_scale (uint32_t scale) {
	native_scale = scale;
}

const char *native_get_failure_reason(void) {
	if (stbi_failure_reason())
		return stbi_failure_reason();
    return jpgd_failure_reason();
}

static inline void clear_alpha(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	memset((void*)pixmap->pixels, col, pixels);
}

static inline void clear_luminance_alpha(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
...
static inline void clear_RGB888(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
...
static inline void draw_RGBA8888(const native_pixmap* pixmap,const unsigned char* buffer){
    int pixels = pixmap->width * pixmap->height;
...
static inline void clear_RGBA8888(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	uint32_t* ptr = (uint32_t*)pixmap->pixels;
...
static inline void clear_RGB565(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	unsigned short* ptr = (unsigned short*)pixmap->pixels;
...
static inline void clear_RGBA4444(const native_pixmap* pixmap, uint32_t col) {
	int pixels = pixmap->width * pixmap->height;
	unsigned short* ptr = (unsigned short*)pixmap->pixels;
	unsigned short l = col & 0xffff;
...
void native_clear(const native_pixmap* pixmap, uint32_t col) {
...
static inline int32_t in_pixmap(const native_pixmap* pixmap, int32_t x, int32_t y) {
	if(x < 0 || y < 0)
		return 0;
...
uint32_t native_get_pixel(const native_pixmap* pixmap, int32_t x, int32_t y) {
	if(!in_pixmap(pixmap, x, y))
		return 0;
	unsigned char* ptr = (unsigned char*)pixmap->pixels + (x + pixmap->width * y) * native_bytes_per_pixel(pixmap->format);
	return to_RGBA8888(pixmap->format, get_pixel_func_ptr(pixmap->format)(ptr));
}

void native_set_pixel(const native_pixmap* pixmap, int32_t x, int32_t y, uint32_t col) {
	if(native_blend) {
		uint32_t dst = native_get_pixel(pixmap, x, y);
		col = blend(col, dst);
		col = to_format(pixmap->format, col);
		set_pixel((unsigned char*)pixmap->pixels, pixmap->width, pixmap->height, native_bytes_per_pixel(pixmap->format), set_pixel_func_ptr(pixmap->format), x, y, col);
	} else {
		col = to_format(pixmap->format, col);
		set_pixel((unsigned char*)pixmap->pixels, pixmap->width, pixmap->height, native_bytes_per_pixel(pixmap->format), set_pixel_func_ptr(pixmap->format), x, y, col);
	}
...
void native_draw_line(const native_pixmap* pixmap, int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t col) {	
    int32_t dy = y1 - y0;
    int32_t dx = x1 - x0;
	int32_t fraction = 0;
    int32_t stepx, stepy;
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
	uint32_t bpp = native_bytes_per_pixel(pixmap->format);
    ...
    if(in_pixmap(pixmap, x0, y0)) {
    	if(native_blend) {
    		col_format = to_format(pixmap->format, blend(col, to_RGBA8888(pixmap->format, pget((unsigned char *)addr))));
    	}
    	pset((unsigned char *)addr, col_format);
    }
    if (dx > dy) {
        fraction = dy - (dx >> 1);
        while (x0 != x1) {
...
static inline void hline(const native_pixmap* pixmap, int32_t x1, int32_t x2, int32_t y, uint32_t col) {
	int32_t tmp = 0;
	set_pixel_func pset = set_pixel_func_ptr(pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(pixmap->format);
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
...
static inline void vline(const native_pixmap* pixmap, int32_t y1, int32_t y2, int32_t x, uint32_t col) {
	int32_t tmp = 0;
	set_pixel_func pset = set_pixel_func_ptr(pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(pixmap->format);
	unsigned char* ptr = (unsigned char*)pixmap->pixels;
...
void native_draw_rect(const native_pixmap* pixmap, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t col) {
...
void native_draw_circle(const native_pixmap* pixmap, int32_t x, int32_t y, uint32_t radius, uint32_t col) {	
    int32_t px = 0;
    int32_t py = radius;
...
void native_fill_rect(const native_pixmap* pixmap, int32_t x, int32_t y, uint32_t width, uint32_t height, uint32_t col) {
...
void native_fill_circle(const native_pixmap* pixmap, int32_t x0, int32_t y0, uint32_t radius, uint32_t col) {
...
void native_fill_triangle(const native_pixmap* pixmap, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, uint32_t col) {
...
static inline void blit_same_size(const native_pixmap* src_pixmap, const native_pixmap* dst_pixmap, 
	...
	uint32_t sbpp = native_bytes_per_pixel(src_pixmap->format);
	uint32_t dbpp = native_bytes_per_pixel(dst_pixmap->format);
	...
	for(;sy < src_y + height; sy++, dy++) {
		...
		for(sx = src_x, dx = dst_x; sx < src_x + width; sx++, dx++) {
			...
			uint32_t src_col = to_RGBA8888(src_pixmap->format, pget((unsigned char*)src_ptr));

			if(native_blend) {
				uint32_t dst_col = to_RGBA8888(dst_pixmap->format, dpget((unsigned char*)dst_ptr));
...
static inline void blit_bilinear(const native_pixmap* src_pixmap, const native_pixmap* dst_pixmap,
	...
	for(;i < dst_height; i++) {
		...
		for(j = 0 ;j < dst_width; j++) {
			...
			c1 = to_RGBA8888(src_pixmap->format, pget((unsigned char*)src_ptr));
			if(sx + 1 < src_width) c2 = to_RGBA8888(src_pixmap->format, pget((unsigned char*)(src_ptr + sbpp))); else c2 = c1;
			if(sy + 1< src_height) c3 = to_RGBA8888(src_pixmap->format, pget((unsigned char*)(src_ptr + spitch))); else c3 = c1;
			if(sx + 1< src_width && sy + 1 < src_height) c4 = to_RGBA8888(src_pixmap->format, pget((unsigned char*)(src_ptr + spitch + sbpp))); else c4 = c1;
...
static inline void blit_linear(const native_pixmap* src_pixmap, const native_pixmap* dst_pixmap,
		   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
		   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
	set_pixel_func pset = set_pixel_func_ptr(dst_pixmap->format);
	get_pixel_func pget = get_pixel_func_ptr(src_pixmap->format);
...
static inline void blit(const native_pixmap* src_pixmap, const native_pixmap* dst_pixmap,
					   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
					   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
	if(native_scale == SCALE_NEAREST)
		blit_linear(src_pixmap, dst_pixmap, src_x, src_y, src_width, src_height, dst_x, dst_y, dst_width, dst_height);
	if(native_scale == SCALE_BILINEAR)
		blit_bilinear(src_pixmap, dst_pixmap, src_x, src_y, src_width, src_height, dst_x, dst_y, dst_width, dst_height);
}

void native_draw_pixmap(const native_pixmap* src_pixmap, const native_pixmap* dst_pixmap,
					   int32_t src_x, int32_t src_y, uint32_t src_width, uint32_t src_height,
					   int32_t dst_x, int32_t dst_y, uint32_t dst_width, uint32_t dst_height) {
...
Java_com_guidebee_game_engine_graphics_NativePixmap_load(JNIEnv* env, jclass clazz,
    jlongArray nativeData, jbyteArray buffer, jint offset, jint len)
{
...
JNIEXPORT jobject JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_newPixmap(JNIEnv* env, jclass clazz,
    jlongArray nativeData, jint width, jint height, jint format)
{
...
Java_com_guidebee_game_engine_graphics_NativePixmap_free(JNIEnv* env, jclass clazz, jlong pixmap)
{
	native_free((native_pixmap*)pixmap);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_clear(JNIEnv* env, jclass clazz,
    jlong pixmap, jint color)
{
...
JNIEXPORT void JNICALL Java_com_guidebee_game_engine_graphics_NativePixmap_drawRGB
  (JNIEnv * env, jclass clazz,jlong pixmap,jintArray buffer)
{
	const unsigned char* p_buffer = (const unsigned char*)env->GetPrimitiveArrayCritical(buffer, 0);
	draw_RGBA8888((native_pixmap*)pixmap,p_buffer);
	env->ReleasePrimitiveArrayCritical(buffer, (char*)p_buffer, 0);
}


JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_setPixel(JNIEnv* env, jclass clazz, jlong pixmap,
...
Java_com_guidebee_game_engine_graphics_NativePixmap_getPixel(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y)
{
	return native_get_pixel((native_pixmap*)pixmap, x, y);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_drawLine(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y, jint x2, jint y2, jint color)
{
	native_draw_line((native_pixmap*)pixmap, x, y, x2, y2, color);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_drawRect(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y, jint width, jint height, jint color)
{
	native_draw_rect((native_pixmap*)pixmap, x, y, width, height, color);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_drawCircle(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y, jint radius, jint color)
{
	native_draw_circle((native_pixmap*)pixmap, x, y, radius, color);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_fillRect(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y, jint width, jint height, jint color)
{
...
JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_fillCircle(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x, jint y, jint radius, jint color)
{
...
JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_fillTriangle(JNIEnv* env, jclass clazz,
    jlong pixmap, jint x1, jint y1, jint x2, jint y2, jint x3, jint y3, jint color)
{
	native_fill_triangle((native_pixmap*)pixmap, x1, y1, x2, y2, x3, y3, color);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_drawPixmap(JNIEnv* env, jclass clazz,
    jlong src, jlong dst, jint srcX, jint srcY, jint srcWidth, jint srcHeight, jint dstX, jint dstY,
    jint dstWidth, jint dstHeight)
{
	native_draw_pixmap((native_pixmap*)src, (native_pixmap*)dst, srcX, srcY, srcWidth, srcHeight, dstX, dstY, dstWidth, dstHeight);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_setBlend(JNIEnv* env, jclass clazz, jint blend)
{
    native_set_blend(blend);
}

JNIEXPORT void JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_setScale(JNIEnv* env, jclass clazz, jint scale)
{
	native_set_scale(scale);
}

JNIEXPORT jstring JNICALL
Java_com_guidebee_game_engine_graphics_NativePixmap_getFailureReason(JNIEnv* env, jclass clazz)
{
