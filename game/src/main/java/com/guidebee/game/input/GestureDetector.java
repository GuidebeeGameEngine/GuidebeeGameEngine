...
import com.guidebee.utils.Timer;

//[------------------------------ MAIN CLASS ----------------------------------]

/**
 * {@link com.guidebee.game.InputProcessor} implementation that detects
 * gestures (tap, long press, fling, pan, zoom, pinch) and hands them to a
...
public class GestureDetector extends InputAdapter {
    ...
    private final Timer.Task longPressTask = new Timer.Task() {
        @Override
        public void run() {
            if (!longPressFired) longPressFired
     ...
     * tapCountInterval=0.4f, longPressDuration=1.1f,
     * maxFlingDelay=0.15f.
     */
    ...
    /**
     * @param halfTapSquareSize half width in pixels of the square around an
     *                          initial touch event, see
     *                          {@link GestureListener#tap(float, float, int, int)}.
     * @param tapCountInterval  time in seconds that must pass for two touch
     *                          down/up sequences to be detected as consecutive taps.
     ...
     */
    public GestureDetector(float halfTapSquareSize, float tapCountInterval,
    ...
    public boolean touchDown(float x, float y, int pointer, int button) {
        ...
        if (pointer == 0) {
            pointer1.set(x, y);
            gestureStartTime = GameEngine.input.getCurrentEventTime();
            tracker.start(x, y, gestureStartTime);
            if (GameEngine.input.isTouched(1)) {
                // Start pinch.
                inTapSquare = false;
                pinching = true;
    ...
    public boolean touchDragged(float x, float y, int pointer) {
        ...
        if (pinching) {
            if (listener != null) {
                boolean result = listener.pinch(initialPointer1,
        ...
        // update tracker
        tracker.update(x, y, GameEngine.input.getCurrentEventTime());

        // check if we are still tapping.
        if (inTapSquare && !isWithinTapSquare(x, y, tapSquareCenterX,
                tapSquareCenterY)) {
            longPressTask.cancel();
            inTapSquare = false;
        }
    ...
    public boolean touchUp(float x, float y, int pointer, int button) {
        ...
        // check if we are still tapping.
        if (inTapSquare && !isWithinTapSquare(x, y, tapSquareCenterX,
                tapSquareCenterY)) inTapSquare = false;
        ...
        if (pinching) {
            ...
            if (pointer == 0) {
                // first pointer has lifted off, set up
                // panning to use the second pointer...
                tracker.start(pointer2.x, pointer2.y,
                        GameEngine.input.getCurrentEventTime());
            } else {
                // second pointer has lifted off, set up
                // panning to use the first pointer...
        ...
        if (wasPanning && !panning) handled
                = listener.panStop(x, y, pointer, button);

        // handle fling
        gestureStartTime = 0;
        long time = GameEngine.input.getCurrentEventTime();
        if (time - tracker.lastTime < maxFlingDelay) {
            tracker.update(x, y, time);
            handled = listener.fling(tracker.getVelocityX(),
                    tracker.getVelocityY(), button) || handled;
        }
        return handled;
    }
    ...
    /**
     * @return whether the user touched the screen long enough
    ...
    /**
     * @param tapCountInterval time in seconds that must pass for
     *                         two touch down/up sequences to be detected
     *                         as consecutive taps.
     */
    ...
    /**
     * Register an instance of this class with a {@link GestureDetector}
     * to receive gestures such as taps, long presses, flings,
     * panning or pinch zooming. Each method returns a boolean indicating
     * if the event should be handed to the next listener (false
    ...
    public static interface GestureListener {
        /**
         * @see com.guidebee.game.InputProcessor#touchDown(int, int, int, int)
         */
        public boolean touchDown(float x, float y, int pointer, int button);

        /**
         * Called when a tap occured. A tap happens if a touch went down
         * on the screen and was lifted again without moving outside
         * of the tap square. The tap square is a rectangular area around the
         * initial touch position as specified on construction
         ...
         * Reports the last known velocity of the finger in
         * pixels per second.
         *
         ...
         * distance is the distance in pixels when the gesture
         * started.
         *
        ...
        /**
         * Called when a user performs a pinch zoom gesture. Reports the
         * initial positions of the two involved fingers and their
         * current positions.
         *
