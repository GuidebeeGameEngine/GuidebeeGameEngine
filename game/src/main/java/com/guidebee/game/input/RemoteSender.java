 ...
 * {@link RemoteInput} at the given ip/port. Instantiate this and
...
public class RemoteSender implements InputProcessor {
    ...
    public RemoteSender(String ip, int port) {
        try {
            ...
            out.writeBoolean(GameEngine.input.isPeripheralAvailable
                    (Peripheral.MultitouchScreen));
            connected = true;
            GameEngine.input.setInputProcessor(this);
        } catch (Exception e) {
            GameEngine.app.log("RemoteSender", "couldn't connect to "
                    + ip + ":" + port);
        }
    }
    ...
    public void sendUpdate() {
        ...
        try {
            out.writeInt(ACCEL);
            out.writeFloat(GameEngine.input.getAccelerometerX());
            out.writeFloat(GameEngine.input.getAccelerometerY());
            out.writeFloat(GameEngine.input.getAccelerometerZ());
            out.writeInt(COMPASS);
            out.writeFloat(GameEngine.input.getAzimuth());
            out.writeFloat(GameEngine.input.getPitch());
            out.writeFloat(GameEngine.input.getRoll());
            out.writeInt(SIZE);
            out.writeFloat(GameEngine.graphics.getWidth());
            out.writeFloat(GameEngine.graphics.getHeight());
        } catch (Throwable t) {
            out = null;
            connected = false;
        }
