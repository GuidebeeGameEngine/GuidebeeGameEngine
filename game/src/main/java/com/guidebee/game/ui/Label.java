...
public class Label extends Widget {
    ...
    private HAlignment lineAlign = Font.HAlignment.LEFT;
    ...
    /**
     * Creates a label, using a {@link LabelStyle} that has a BitmapFont
     * with the specified name from the skin and the specified
     * color.
     */
     ...
     * the specified name and the specified color from the
     * skin.
     */
    ...
    /**
     * Returns the label's style. Modifying the returned style may not have an effect
     * until {@link #setStyle(LabelStyle)} is
     * called.
     */
    ...
    public void layout() {
        ...
        if (ellipse && width < bounds.width) {
            ...
            if (width > ellipseWidth) {
                text.append(this.text, 0, font.computeVisibleGlyphs(this.text, 0,
                        this.text.length, width - ellipseWidth));
                text.append("...");
            }
        } else
            text = this.text;
    ...
    public float getPrefWidth() {
        ...
        Drawable background = style.background;
        if (background != null) width += background.getLeftWidth()
    ...
    public float getPrefHeight() {
        ...
        Drawable background = style.background;
        if (background != null) height += background.getTopHeight()
     ...
     * The preferred width of the label will be 0, it is expected
     * that the something external will set the width of the label.
     * Default is false.
     * <p/>
     * When wrap is enabled, the label's preferred height depends on the
     * width of the label. In some cases the parent of the label
     * will need to layout twice: once to set the width of the label and a
     * second time to adjust to the label's new preferred
     * height.
     */
    ...
    /**
     * @param alignment Aligns each line of text horizontally and all
     *                  the text vertically.
     * @see Align
     */
    ...
    public void setAlignment(int labelAlign, int lineAlign) {
        ...
        if ((lineAlign & Align.left) != 0)
            this.lineAlign = Font.HAlignment.LEFT;
        else if ((lineAlign & Align.right) != 0)
            this.lineAlign = Font.HAlignment.RIGHT;
        else
            this.lineAlign = Font.HAlignment.CENTER;
    ...
    /**
     * When true the text will be truncated with an ellipse if it does not
     * fit within the width of the label. Default is false.
     */
