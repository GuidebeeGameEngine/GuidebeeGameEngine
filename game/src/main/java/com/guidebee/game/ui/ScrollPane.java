 ...
 * height is less than the size of this scroll pane,
 * it is set to the size of this scroll pane. Scrollbars appear when the widget
 * is larger than the scroll pane.
 * <p/>
 * The scroll pane's preferred size is that of the child widget. At this size,
 * the child widget will not need to scroll, so the
 * scroll pane is typically sized by ignoring the preferred size in one or both
 * directions.
 *
 * @author mzechner
 * @author Nathan Sweet
...
public class ScrollPane extends WidgetGroup {
        ...
        addCaptureListener(new InputListener() {
            ...
            public void touchDragged(InputEvent event, float x, float y, int pointer) {
                ...
                if (touchScrollH) {
                    ...
                    scrollH = Math.min(hScrollBounds.x + hScrollBounds.width - hKnobBounds.width,
                            scrollH);
                    ...
                    scrollV = Math.min(vScrollBounds.y + vScrollBounds.height - vKnobBounds.height,
                            scrollV);
    ...
    public void act(float delta) {
        ...
        if (smoothScrolling && flingTimer <= 0 && !touchScrollH && !touchScrollV && !panning) {
            if (visualAmountX != amountX) {
                if (visualAmountX < amountX)
                    visualScrollX(Math.min(amountX, visualAmountX + Math.max(200 * delta,
                            (amountX - visualAmountX) * 7 * delta)));
                else
                    visualScrollX(Math.max(amountX, visualAmountX - Math.max(200 * delta,
                            (visualAmountX - amountX) * 7 * delta)));
            }
            if (visualAmountY != amountY) {
                if (visualAmountY < amountY)
                    visualScrollY(Math.min(amountY, visualAmountY + Math.max(200 * delta,
                            (amountY - visualAmountY) * 7 * delta)));
                else
                    visualScrollY(Math.max(amountY, visualAmountY - Math.max(200 * delta,
                            (visualAmountY - amountY) * 7 * delta)));
            }
        } else {
    ...
    public void layout() {
        ...
        if (style.hScroll != null) scrollbarHeight = Math.max(scrollbarHeight,
                style.hScroll.getMinHeight());
        float scrollbarWidth = 0;
        if (vScrollKnob != null) scrollbarWidth = vScrollKnob.getMinWidth();
        if (style.vScroll != null) scrollbarWidth = Math.max(scrollbarWidth,
        ...
        if (!fade) {
            // Check again, now taking into account the area that's taken up
            // by any enabled scrollbars.
            if (scrollY) {
        ...
        if (scrollX) {
            if (hScrollKnob != null) {
                ...
                // the small gap where the two scroll bars intersect might have to
        ...
        if (scrollY) {
            if (vScrollKnob != null) {
                ...
                vKnobBounds.y = vScrollBounds.y + (int) ((vScrollBounds.height - vKnobBounds.height)
                        * (1 - getScrollPercentY()));
            } else {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        // Calculate the widget's position depending on the scroll state
        // and available widget area.
        ...
        if (!fadeScrollBars && scrollbarsOnTop && scrollX) {
            ...
            if (style.hScroll != null) scrollbarHeight
                    = Math.max(scrollbarHeight, style.hScroll.getMinHeight());
            y += scrollbarHeight;
        }
        ...
        if (scrollX && scrollY) {
            if (style.corner != null) {
                style.corner
                        .draw(batch, hScrollBounds.x + hScrollBounds.width,
    ...
    public float getPrefWidth() {
        if (widget instanceof Layout) {
            ...
            if (style.background != null) width += style.background.getLeftWidth()
     ...
     * centered vertically in the scroll pane, if possible.
    ...
    /**
     * Disables scrolling in a direction. The widget will be sized to the FlickScrollPane
     * in the disabled direction.
     */
     ...
     * and will animate back to its bounds when
     ...
     * returns to the widget's bounds in seconds. Default
     * is 50, 30, 200.
     */
     ...
     * (for flick scroll) in a direction, even if the contents
     ...
     * scrollbars. When true, the widget is clipped to
     * the entire scroll pane bounds and the scrollbars are drawn on top of the widget.
     * If {@link #setFadeScrollBars(boolean)} is
     ...
     * {@link #getMaxY()}. If false, the scroll knobs are sized
     * based on {@link Drawable#getMinWidth()} or {@link Drawable#getMinHeight()}.
     * Default is true.
     */
    ...
    static public class ScrollPaneStyle {
        ...
        public ScrollPaneStyle(Drawable background, Drawable hScroll,
                               Drawable hScrollKnob, Drawable vScroll,
