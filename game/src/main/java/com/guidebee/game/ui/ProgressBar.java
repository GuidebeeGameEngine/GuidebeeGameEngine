 ...
 * activity or a value within given range. The progress bar
 * has a range (min, max) and a stepping between each value it represents.
 * The percentage of completeness typically starts out as
 * an empty progress bar and gradually becomes filled in as the task or variable
 * value progresses.
 * <p/>
 * {@link com.guidebee.game.ui.ChangeListener.ChangeEvent} is fired
 * when the progress bar knob is moved. Cancelling the event will move the knob
 * to where it was
 * previously.
 * <p/>
 * The preferred height of a progress bar is determined by the larger of the knob
 * and background. The preferred width of progress
...
public class ProgressBar extends Widget implements Disableable {
    ...
    public ProgressBar(float min, float max, float stepSize,
    ...
    /**
     * Creates a new progress bar. It's width is determined by the given prefWidth
     * parameter, its height is determined by the
     * maximum of the height of either the progress bar
     * {@link com.guidebee.game.graphics.NinePatch} or progress bar
     * handle {@link com.guidebee.game.graphics.TextureRegion}. The min and
     * max values determine the range the values of this progress bar can take on,
     * the stepSize parameter specifies the distance
     * between individual values.
     * <p/>
     * E.g. min could be 4, max could be 10 and stepSize could be 0.2, giving you
     * a total of 30 values, 4.0 4.2, 4.4 and so on.
     *
    ...
    /**
     * Returns the progress bar's style. Modifying the returned style may
     * not have an effect until
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        final Drawable knob = (disabled && style.disabledKnob != null)
                ? style.disabledKnob : style.knob;
        final Drawable bg = (disabled && style.disabledBackground != null)
                ? style.disabledBackground : style.background;
        final Drawable knobBefore = (disabled && style.disabledKnobBefore != null)
                ? style.disabledKnobBefore : style.knobBefore;
        final Drawable knobAfter = (disabled && style.disabledKnobAfter != null)
                ? style.disabledKnobAfter : style.knobAfter;
        ...
        if (vertical) {
            bg.draw(batch, x + (int) ((width - bg.getMinWidth()) * 0.5f),
                    y, bg.getMinWidth(), height);
            ...
            if (knobBefore != null) {
                ...
                knobBefore.draw(batch, x + (int) ((width - knobBefore.getMinWidth()) * 0.5f),
                        y + offset, knobBefore.getMinWidth(),
                        (int) (position + knobHeightHalf));
            }
            if (knobAfter != null) {
                knobAfter.draw(batch, x + (int) ((width - knobAfter.getMinWidth()) * 0.5f),
                        y + (int) (position + knobHeightHalf),
                        knobAfter.getMinWidth(), height - (int) (position + knobHeightHalf));
            }
            if (knob != null)
                knob.draw(batch, x + (int) ((width - knobWidth) * 0.5f), (int) (y + position),
            ...
            if (knobBefore != null) {
                ...
                knobBefore.draw(batch, x + offset,
                        y + (int) ((height - knobBefore.getMinHeight()) * 0.5f),
                        (int) (position + knobWidthHalf), knobBefore.getMinHeight());
            }
            if (knobAfter != null) {
                knobAfter.draw(batch, x + (int) (position + knobWidthHalf),
                        y + (int) ((height - knobAfter.getMinHeight()) * 0.5f),
                        width - (int) (position + knobWidthHalf),
                        knobAfter.getMinHeight());
            }
            if (knob != null)
                knob.draw(batch, (int) (x + position),
                        (int) (y + (height - knobHeight) * 0.5f), knobWidth, knobHeight);
        }
    }
    ...
    public float getVisualValue() {
        if (animateTime > 0)
            return animateInterpolation.apply(animateFromValue, value,
     ...
     * clamped to the minimum and maximum values.
     * {@link #clamp(float)} can be overridden to allow values outside of
     * the progress bar's min/max range.
     *
    ...
    public boolean setValue(float value) {
        ...
        if (!shiftIgnoresSnap || (!GameEngine.input.isKeyPressed(Keys.SHIFT_LEFT)
     ...
     * to allow a range different from the progress
     * bar knob's range.
     */
    ...
    /**
     * Sets the range of this progress bar. The progress bar's current
    ...
    /**
     * Will make this progress bar snap to the specified values, if the knob
