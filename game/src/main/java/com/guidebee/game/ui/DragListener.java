...
/**
 * Detects mouse or finger touch drags on an component. A touch must go down over the
 * component and a drag won't start until it is moved
 * outside the {@link #setTapSquareSize(float) tap square}. Any touch
 * (not just the first) will trigger this listener. While
...
public class DragListener extends InputListener {
    ...
    public void touchDragged(InputEvent event, float x, float y, int pointer) {
        ...
        if (!dragging && (Math.abs(touchDownX - x) > tapSquareSize
    ...
    /* If a drag is in progress, no further drag methods will be called
    ...
    /**
     * Returns the amount on the x axis that the touch has been dragged
    ...
    /**
     * Returns the amount on the y axis that the touch has been dragged
     * since the last drag event.
     */
     ...
     * Default is {@link Buttons#LEFT}. Use -1 for any button.
     */
