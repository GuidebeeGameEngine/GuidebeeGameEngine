 ...
 * vertically. The user may resize the widgets. The
 * child widgets are always sized to fill their half of the splitpane.
 * <p/>
 * The preferred size of a splitpane is that of the child widgets and the size of
 * the {@link SplitPaneStyle#handle}. The widgets
 * are sized depending on the splitpane's size and the {@link #setSplitAmount(float)
 * split position}.
 *
 * @author mzechner
 * @author Nathan Sweet
...
public class SplitPane extends WidgetGroup {
     ...
     * an effect until {@link #setStyle(SplitPaneStyle)}
     * is called.
     */
    ...
    public void layout() {
        ...
        if (secondWidget != null) {
            ...
            secondWidget.setBounds(secondWidgetBounds.x, secondWidgetBounds.y,
                    secondWidgetBounds.width, secondWidgetBounds.height);
    ...
    public float getPrefWidth() {
        float width = 0;
        if (firstWidget != null)
            width = firstWidget instanceof Layout ? ((Layout) firstWidget).getPrefWidth()
                    : firstWidget.getWidth();
        if (secondWidget != null)
            width += secondWidget instanceof Layout ? ((Layout) secondWidget).getPrefWidth()
    ...
    public float getPrefHeight() {
        float height = 0;
        if (firstWidget != null)
            height = firstWidget instanceof Layout ? ((Layout) firstWidget).getPrefHeight()
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        if (firstWidget != null) {
            getWindow().calculateScissors(firstWidgetBounds, firstScissors);
        ...
        if (secondWidget != null) {
            getWindow().calculateScissors(secondWidgetBounds, secondScissors);
    ...
    public void setMinSplitAmount(float minAmount) {
        if (minAmount < 0)
            throw new GameEngineRuntimeException("minAmount has to be >= 0");
        if (minAmount >= maxAmount)
            throw new GameEngineRuntimeException("minAmount has to be < maxAmount");
        this.minAmount = minAmount;
    }

    public void setMaxSplitAmount(float maxAmount) {
        if (maxAmount > 1)
            throw new GameEngineRuntimeException("maxAmount has to be >= 0");
        if (maxAmount <= minAmount)
            throw new GameEngineRuntimeException("maxAmount has to be > minAmount");
