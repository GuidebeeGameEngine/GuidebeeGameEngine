...
public class TextArea extends TextField {
    ...
    public float getPrefHeight() {
        if (prefRows <= 0) {
            ...
            if (style.background != null) {
                prefHeight = Math.max(prefHeight + style.background.getBottomHeight()
    ...
    public void moveCursorLine(int line) {
        if (line < 0) {
            ...
            if (moveOffset < 0) {
                moveOffset = linesBreak.size <= cursorLine * 2
                        ? 0 : glyphPositions.get(cursor)
                        - glyphPositions.get(linesBreak.get(cursorLine * 2));
            }
            cursorLine = line;
            cursor = cursorLine * 2 >= linesBreak.size
                    ? text.length() : linesBreak.get(cursorLine * 2);
    ...
    protected void sizeChanged() {
        ...
        float availableHeight = getHeight() - (background == null
                ? 0 : background.getBottomHeight() + background.getTopHeight());
    ...
    protected void drawSelection(Drawable selection, Batch batch,
        ...
        while (i + 1 < linesBreak.size
            ...
            if (!((minIndex < lineStart && minIndex < lineEnd
                    ...
                    && minIndex > lineEnd && maxIndex > lineStart
                    && maxIndex > lineEnd))) {
    ...
    protected void drawText(Batch batch, BitmapFont font, float x, float y) {
        ...
        for (int i = firstLineShowing * 2;
            ...
            font.draw(batch, displayText, x, y + offsetY,
                    linesBreak.items[i], linesBreak.items[i + 1]);
            offsetY -= font.getLineHeight();
        }
    }
    ...
    protected void drawCursor(Drawable cursorPatch, Batch batch,
        ...
        float textOffset = cursor >= glyphPositions.size
                || cursorLine * 2 >= linesBreak.size ? 0 : glyphPositions.get(cursor)
                - glyphPositions.get(linesBreak.items[cursorLine * 2]);
        cursorPatch.draw(batch, x + textOffset,
                y - font.getDescent() / 2 - (cursorLine - firstLineShowing + 1)
                        * font.getLineHeight(), cursorPatch.getMinWidth(),
                font.getLineHeight());
    }
    ...
    protected void calculateOffsets() {
        ...
        if (!this.text.equals(lastText)) {
            this.lastText = text;
            Font.TextBounds bounds = new Font.TextBounds();
            BitmapFont font = style.font;
    ...
    /**
     * IInput listener for the text area *
     */
