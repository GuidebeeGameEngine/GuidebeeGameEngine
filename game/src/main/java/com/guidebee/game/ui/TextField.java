 ...
 * {@link TextFieldStyle#font} and {@link TextFieldStyle#background}.
 ...
 * and paste any text in the clipboard when ctrl+v is
 * pressed. IClipboard functionality is provided via the
 * {@link com.guidebee.utils.Clipboard} interface. Currently there
 * are two standard
 * implementations, one for the desktop and one for Android. The Android clipboard
 * is a stub, as copy & pasting on Android is not
 * supported yet.
 * <p/>
 * The text field allows you to specify an {@link OnscreenKeyboard} for displaying a
 * softkeyboard and piping all key events
 * generated by the keyboard to the text field. There are two standard
 * implementations, one for the desktop and one for Android.
 * The desktop keyboard is a stub, as a softkeyboard is not needed on the desktop.
 * The Android {@link OnscreenKeyboard}
...
public class TextField extends Widget implements Disableable {
    ...
    public TextField(String text, TextFieldStyle style) {
        setStyle(style);
        clipboard = GameEngine.app.getClipboard();
        initialize();
        setText(text);
     ...
     * not in the font, a space will be displayed instead.
     * When true (the default), characters not in the font are stripped by setText.
     * Characters not in the font are always stripped
    ...
    /**
     * Returns the text field's style. Modifying the returned style may not
     * have an effect until {@link #setStyle(TextFieldStyle)}
     * is called.
     */
    ...
    protected void calculateOffsets() {
        ...
        // Check if the cursor has gone out the left or right side of the visible
        // area and adjust renderoffset.
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        final Drawable background = (disabled && style.disabledBackground != null)
                ? style.disabledBackground
                : ((focused && style.focusedBackground != null)
        ...
        if (displayText.length() == 0) {
            if (!focused && messageText != null) {
                if (style.messageFontColor != null) {
                    font.setColor(style.messageFontColor.r, style.messageFontColor.g,
     ...
     */
    protected void drawSelection(Drawable selection, Batch batch, BitmapFont font,
                                 float x, float y) {
    ...
    protected void drawText(Batch batch, BitmapFont font, float x, float y) {
        font.draw(batch, displayText, x + textOffset, y,
                visibleTextStart, visibleTextEnd);
    }
    ...
    protected void drawCursor(Drawable cursorPatch, Batch batch,
        ...
        cursorPatch.draw(batch, x + textOffset + glyphPositions.get(cursor)
                        - glyphPositions.items[visibleTextStart] - 1, y
                        - textHeight - font.getDescent(), cursorPatch.getMinWidth(),
                textHeight + font.getDescent() / 2);
    }
    ...
    void updateDisplayText() {
        ...
        if (passwordMode && font.containsCharacter(passwordCharacter)) {
            if (passwordBuffer == null) passwordBuffer
                    = new StringBuilder(newDisplayText.length());
    ...
    /**
     * Copies the contents of this TextField to the {@link com.guidebee.utils.Clipboard}
     * implementation set on this TextField.
     */
    public void copy() {
        if (hasSelection && !passwordMode) {
            clipboard.setContents(text.substring(Math.min(cursor, selectionStart),
                    Math.max(cursor, selectionStart)));
        }
    }

    /**
     * Copies the selected contents of this TextField to the {@link com.guidebee.utils.Clipboard}
    ...
    /**
     * Pastes the content of the {@link com.guidebee.utils.Clipboard} implementation set on this
     ...
     * in the {@link BitmapFont}. Default is 149
     * (bullet).
     */
     ...
     * Just uses
     * {@link com.guidebee.game.Input#setOnscreenKeyboardVisible(boolean)}
     * as appropriate. Might overlap your actual rendering, so use with care!
     *
     * @author mzechner
     */
    static public class DefaultOnscreenKeyboard implements OnscreenKeyboard {
        @Override
        public void show(boolean visible) {
            GameEngine.input.setOnscreenKeyboardVisible(visible);
        }
    }
    ...
    public class TextFieldClickListener extends ClickListener {
        ...
        public boolean keyDown(InputEvent event, int keycode) {
            ...
            if (Utils.shift()) {
                ...
                if (keycode == Input.Keys.FORWARD_DEL && hasSelection) {
                    copy();
                    delete(); // cut
                }
                selection:
        ...
        public boolean keyTyped(InputEvent event, char character) {
            ...
            if ((character == TAB || character == ENTER_ANDROID) && focusTraversal) {
                ...
                boolean add = style.font.containsCharacter(character)
                        || (writeEnters && (character == ENTER_ANDROID
                        || character == ENTER_DESKTOP));
                ...
                if (add || remove) {
                    ...
                    if (add && !remove) {
                        ...
                        boolean isEnter = character == ENTER_DESKTOP
    ...
    static public class TextFieldStyle {
        ...
        public TextFieldStyle(BitmapFont font, Color fontColor, Drawable cursor,
        ...
        public TextFieldStyle(TextFieldStyle style) {
            ...
            if (style.messageFontColor != null) this.messageFontColor
                    = new Color(style.messageFontColor);
            this.background = style.background;
            this.focusedBackground = style.focusedBackground;
            this.disabledBackground = style.disabledBackground;
            this.cursor = style.cursor;
            this.font = style.font;
            if (style.fontColor != null) this.fontColor
                    = new Color(style.fontColor);
            if (style.focusedFontColor != null) this.focusedFontColor
                    = new Color(style.focusedFontColor);
