 ...
 * centered on the touchpad, and its size determined by the
 ...
 * fired when the touchpad knob is moved. Cancelling the event will move the
 * knob to where it was
 * previously.
 *
 * @author Josh Street
 */
public class Touchpad extends Widget {
    ...
    void calculatePositionAndValue(float x, float y, boolean isTouchUp) {
        ...
        if (!isTouchUp) {
            if (!deadzoneBounds.contains(x, y)) {
                knobPercent.set((x - centerX) / knobBounds.radius,
                        (y - centerY) / knobBounds.radius);
                float length = knobPercent.len();
                if (length > 1) knobPercent.scl(1 / length);
                if (knobBounds.contains(x, y)) {
                    knobPosition.set(x, y);
                } else {
                    knobPosition.set(knobPercent).nor().
                            scl(knobBounds.radius).add(knobBounds.x, knobBounds.y);
                }
            }
        }
     ...
     * effect until {@link #setStyle(TouchpadStyle)} is
     * called.
     */
    ...
    public void layout() {
        ...
        if (style.knob != null) radius -= Math.max(style.knob.getMinWidth(),
                style.knob.getMinHeight()) / 2;
