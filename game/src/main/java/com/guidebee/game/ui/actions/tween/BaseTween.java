...
public abstract class BaseTween<T> {
    ...
    public T repeat(int count, float delay) {
        if (isStarted)
            throw new RuntimeException("You can't change the repetitions of a " +
                    "tween or timeline once it is started");
        repeatCnt = count;
    ...
    public T repeatYoyo(int count, float delay) {
        if (isStarted)
            throw new RuntimeException("You can't change the repetitions of a tween " +
                    "or timeline once it is started");
        repeatCnt = count;
        repeatDelay = delay >= 0 ? delay : 0;
        isYoyo = true;
        return (T) this;
     ...
     * and add more triggers, use the setCallbackTriggers() method.
     *
     * @see TweenCallback
     */
    ...
    private void testRelaunch() {
        if (!isIterationStep && repeatCnt >= 0
                && step < 0 && currentTime + deltaTime >= 0) {
            assert step == -1;
            isIterationStep = true;
            step = 0;
        ...
        } else if (!isIterationStep && repeatCnt >= 0
                && step > repeatCnt * 2 && currentTime + deltaTime < 0) {
