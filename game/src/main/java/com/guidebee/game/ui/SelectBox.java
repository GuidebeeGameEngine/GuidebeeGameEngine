 ...
 * of values from a list. When inactive, the selected
 * value is displayed. When activated, it shows the list of values that may be
 * selected.
 * <p/>
 * {@link com.guidebee.game.ui.ChangeListener.ChangeEvent} is fired
 * when the selectbox selection changes.
 * <p/>
 * The preferred size of the select box is determined by the maximum text bounds
 * of the items and the size of the
...
public class SelectBox<T> extends Widget implements Disableable {
    ...
    /**
     * @return Max number of items to display when the box is opened,
     * or <= 0 to display them all.
     */
    ...
    /**
     * Returns the select box's style. Modifying the returned style may not
     * have an effect until {@link #setStyle(SelectBoxStyle)}
     * is called.
     */
    ...
    public void layout() {
        ...
        prefHeight = Math.max(bg.getTopHeight() + bg.getBottomHeight()
                        + font.getCapHeight() - font.getDescent() * 2,
                bg.getMinHeight());

        float maxItemWidth = 0;
        for (int i = 0; i < items.size; i++)
            maxItemWidth = Math.max(font.getBounds(items.get(i).toString()).width,
                    maxItemWidth);
        ...
        prefWidth = Math.max(
                ...
                maxItemWidth
                        ...
                        ? 0 : scrollStyle.background.getLeftWidth()
                        + scrollStyle.background.getRightWidth())
                        + listStyle.selection.getLeftWidth()
                        + listStyle.selection.getRightWidth()
                        + Math.max(style.scrollStyle.vScroll != null
                                ? style.scrollStyle.vScroll.getMinWidth() : 0,
                        style.scrollStyle.vScrollKnob != null
                                ? style.scrollStyle.vScrollKnob.getMinWidth() : 0));
    }
     ...
     * use {@link SelectBox#getSelection()}
     */
    ...
    /**
     * Returns the scroll pane containing the list that is shown when the
     * select box is open.
     */
    ...
    class ListScroll extends ScrollPane {
        ...
        public ListScroll() {
            ...
            list.addListener(new InputListener() {
                public boolean mouseMoved(InputEvent event, float x, float y) {
                    list.setSelectedIndex(Math.min(items.size - 1,
                            (int) ((list.getHeight() - y) / list.getItemHeight())));
                    return true;
                }
            });
        ...
        public void show(UIWindow stage) {
            ...
            if (scrollPaneBackground != null)
                height += scrollPaneBackground.getTopHeight()
                        + scrollPaneBackground.getBottomHeight();
            Drawable listBackground = list.getStyle().background;
            if (listBackground != null) height += listBackground.getTopHeight()
    ...
    static public class SelectBoxStyle {
        ...
        public SelectBoxStyle(BitmapFont font, Color fontColor, Drawable background,
        ...
        public SelectBoxStyle(SelectBoxStyle style) {
            ...
            if (style.disabledFontColor != null) this.disabledFontColor
                    = new Color(style.disabledFontColor);
