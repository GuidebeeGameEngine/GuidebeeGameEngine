...
public class Selection<T> implements Disableable, Iterable<T> {
    ...
    /**
     * Selects or deselects the specified item based on how the selection
     * is configured, whether ctrl is currently pressed, etc.
    ...
    public void choose(T item) {
        ...
        snapshot();
        if ((toggle || (!required && selected.size == 1)
                || Utils.ctrl()) && selected.contains(item)) {
            if (required && selected.size == 1) return;
            selected.remove(item);
            lastSelected = null;
        } else {
            boolean modified = false;
            if (!multiple || (!toggle && !Utils.ctrl())) {
    ...
    /**
     * Makes a best effort to return the last item selected, else returns
     * an arbitrary item or null if the selection is empty.
     */
    ...
    /**
     * If true, allows {@link #choose(Object)} to select multiple items.
     * Default is false.
     */
