 ...
 * by a subclass. The default minimum size
 * returns the preferred size, so a subclass may choose to return 0 if it wants to
 * allow itself to be sized smaller. The default
 * maximum size is 0, which means no maximum size.
 * <p/>
 * See {@link Layout} for details on how a widget should participate in layout.
 * A widget's mutator methods should call
