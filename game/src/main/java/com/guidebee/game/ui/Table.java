...
/**
 * A group that sizes and positions children using table constraints. By default,
 * {@link #getTouchable()} is
...
public class Table extends WidgetGroup {
    ...
    Value padTop = Value.zero, padLeft = Value.zero, padBottom = Value.zero,
            padRight = Value.zero;
    ...
    /**
     * Creates a table with a skin, which enables the {@link #add(String)}
     * and {@link #add(String, String)} methods to be used.
     */
     ...
     * Default implementation draws the background
     * drawable.
     */
     ...
     * to match the background. This may only be called
    ...
    /**
     * Sets the background drawable and adjusts the table's padding to match the
     * background.
     *
     * @see #setBackground(Drawable, boolean)
     */
    ...
    /**
     * Sets the background drawable and, if adjustPadding is true, sets the table's
     * padding to {@link Drawable#getBottomHeight()} ,
    ...
    public void setBackground(Drawable background, boolean adjustPadding) {
        ...
        if (adjustPadding) {
            if (background == null)
                ...
                pad(background.getTopHeight(), background.getLeftWidth(),
                        background.getBottomHeight(), background.getRightWidth());
            invalidate();
        }
    }
        ...
        if (cellCount > 0) {
            ...
            if (cell.row > 0) {
                ...
                for (int i = cellCount - 1; i >= 0; i--) {
                    ...
                    for (int column = other.column, nn = column + other.colspan;
                         column < nn; column++) {
    ...
    /**
     * Adds a new cell with a label. This may only be called if {@link Table#Table(Skin)}
     * or {@link #setSkin(Skin)} was used.
     */
    ...
    public Cell<Label> add(String text, String labelStyleName) {
        if (skin == null) throw new IllegalStateException("Table must have a skin set" +
                " to use this method.");
    ...
    public Cell<Label> add(String text, String fontName, String colorName) {
        if (skin == null)
            throw new IllegalStateException("Table must have a skin set to use this method.");
        return add(new Label(text, new Label.LabelStyle(skin.getFont(fontName),
                skin.getColor(colorName))));
    }
     ...
     * cell values that will be used as the defaults
     ...
     * in the specified column. Columns are indexed starting
     * at 0.
     */
    ...
    private void computeSize() {
        ...
        float[] columnMinWidth = this.columnMinWidth
                = ensureSize(this.columnMinWidth, columns);
        float[] rowMinHeight = this.rowMinHeight
                = ensureSize(this.rowMinHeight, rows);
        float[] columnPrefWidth = this.columnPrefWidth
                = ensureSize(this.columnPrefWidth, columns);
        float[] rowPrefHeight = this.rowPrefHeight
                = ensureSize(this.rowPrefHeight, rows);
        float[] columnWidth = this.columnWidth
                = ensureSize(this.columnWidth, columns);
        float[] rowHeight = this.rowHeight
                = ensureSize(this.rowHeight, rows);
        float[] expandWidth = this.expandWidth
                = ensureSize(this.expandWidth, columns);
        float[] expandHeight = this.expandHeight
                = ensureSize(this.expandHeight, rows);
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            if (c.cellAboveIndex != -1) {
                ...
                c.computedPadTop += Math.max(0,
                        c.spaceTop.get(a) - above.spaceBottom.get(a));
            }
            float spaceRight = c.spaceRight.get(a);
            c.computedPadRight = c.padRight.get(a)
                    + ((column + colspan) == columns ? 0 : spaceRight);
            c.computedPadBottom = c.padBottom.get(a)
                    + (row == rows - 1 ? 0 : c.spaceBottom.get(a));
            ...
            if (colspan == 1) { // Spanned column min and pref width is added later.
                ...
                columnPrefWidth[column] = Math.max(columnPrefWidth[column],
                        prefWidth + hpadding);
                columnMinWidth[column] = Math.max(columnMinWidth[column],
                        minWidth + hpadding);
            }
            float vpadding = c.computedPadTop + c.computedPadBottom;
            rowPrefHeight[row] = Math.max(rowPrefHeight[row],
                    prefHeight + vpadding);
            rowMinHeight[row] = Math.max(rowMinHeight[row],
                    minHeight + vpadding);
        }
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            for (int ii = column, nn = ii + colspan; ii < nn; ii++) {
                float ratio = totalExpandWidth == 0
                        ? 1f / colspan : expandWidth[ii] / totalExpandWidth;
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            if (c.uniformX == Boolean.TRUE && c.colspan == 1) {
                float hpadding = c.computedPadLeft + c.computedPadRight;
                uniformMinWidth = Math.max(uniformMinWidth,
                        columnMinWidth[c.column] - hpadding);
                uniformPrefWidth = Math.max(uniformPrefWidth,
                        columnPrefWidth[c.column] - hpadding);
            }
            if (c.uniformY == Boolean.TRUE) {
                float vpadding = c.computedPadTop + c.computedPadBottom;
                uniformMinHeight = Math.max(uniformMinHeight,
                        rowMinHeight[c.row] - vpadding);
                uniformPrefHeight = Math.max(uniformPrefHeight,
                        rowPrefHeight[c.row] - vpadding);
            }
        }
        ...
        if (uniformPrefWidth > 0 || uniformPrefHeight > 0) {
            for (int i = 0; i < cellCount; i++) {
                ...
                if (uniformPrefWidth > 0
                        && c.uniformX == Boolean.TRUE && c.colspan == 1) {
                    float hpadding = c.computedPadLeft + c.computedPadRight;
     ...
     * each child. The values given are the position
    ...
    private void layout(float layoutX, float layoutY,
        ...
        if (totalGrowWidth == 0)
            ...
            float extraWidth = Math.min(totalGrowWidth,
                    Math.max(0, layoutWidth - tableMinWidth));
            columnWeightedWidth = Table.columnWeightedWidth
                    = ensureSize(Table.columnWeightedWidth, columns);
            float[] columnMinWidth = this.columnMinWidth,
        ...
        if (totalGrowHeight == 0)
            ...
            rowWeightedHeight = Table.rowWeightedHeight
                    = ensureSize(Table.rowWeightedHeight, rows);
            float extraHeight = Math.min(totalGrowHeight, Math.max(0,
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            if (c.colspan == 1) columnWidth[column]
                    = Math.max(columnWidth[column], spannedWeightedWidth);
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            if (debug == Debug.cell || debug == Debug.all) {
                addDebugRect(currentX, currentY + c.computedPadTop, spannedCellWidth,
                        rowHeight[c.row] - c.computedPadTop
    ...
    private void drawDebugRects(ShapeRenderer shapes) {
        ...
        for (int i = 0, n = debugRects.size; i < n; i++) {
            ...
            shapes.rect(x + debugRect.x, y + debugRect.y,
                    debugRect.width, debugRect.height);
        }
    }
