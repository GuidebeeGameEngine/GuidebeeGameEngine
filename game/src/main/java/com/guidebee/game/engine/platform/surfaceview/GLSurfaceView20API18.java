...
//[------------------------------ MAIN CLASS ----------------------------------]
/**
 * <b>This class is a slightly modified copy of GLSurfaceView20 which extends
 * GLSurfaceViewAPI18 instead of the classic GLSurfaceView.
 * Especially it is intended to be used on Android 2.x if you need proper
 * support for onAttachedToWindow and onDetachedFromWindow methods.<b>
 * <p>It demonstrates how to perform OpenGL ES 2.0 rendering into a GL Surface.
 ...
 * EGLConfig that supports 2.0. This is done by providing a
 * config specification to eglChooseConfig() that has the attribute EGL10.
 * ELG_RENDERABLE_TYPE containing the EGL_OPENGL_ES2_BIT
 * flag set. See ConfigChooser class definition below.
 * <p/>
 * - The class must select the surface's format, then choose an EGLConfig
 * that matches it exactly (with regards to
 * red/green/blue/alpha channels bit depths). Failure to do so would
...
public class GLSurfaceView20API18 extends GLSurfaceViewAPI18 {
    ...
    public GLSurfaceView20API18(Context context, boolean translucent, int depth,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ResolutionStrategy.MeasuredDimension measures
                = resolutionStrategy.calcMeasures(widthMeasureSpec,
                heightMeasureSpec);
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        // add this line, the IME can show the selectable words
        // when use chinese input method editor.
        if (outAttrs != null) {
        ...
        BaseInputConnection connection = new BaseInputConnection(this, false) {
            ...
            public boolean deleteSurroundingText(int beforeLength, int afterLength) {
                ...
                if (sdkVersion >= 16) {
					 ...
					 * Instead, they send beforeLength = 1, afterLength = 0. So,
            ...
            private void sendDownUpKeyEventForBackwardCompatibility(final int code) {
                ...
                super.sendKeyEvent(new KeyEvent(eventTime, eventTime,
                        KeyEvent.ACTION_DOWN, code, 0, 0,
                        KeyCharacterMap.VIRTUAL_KEYBOARD, 0,
                        KeyEvent.FLAG_SOFT_KEYBOARD | KeyEvent.FLAG_KEEP_TOUCH_MODE));
                super.sendKeyEvent(new KeyEvent(SystemClock.uptimeMillis(),
    ...
    private void init(boolean translucent, int depth, int stencil) {
		 ...
		 * By default, GLSurfaceView() creates a RGB_565 opaque surface.
		  * If we want a translucent one, we should change the
		 * surface's format here, using PixelFormat.TRANSLUCENT for GL
		 * Surfaces is interpreted as any 32-bit surface with alpha by
		 * SurfaceFlinger.
		 */
        if (translucent) {
		...
		/*
		 * Setup the context factory for 2.0 rendering. See
		 ...
		 * surface exactly. This is going to be done in our custom
    ...
    private static class ConfigChooser implements GLSurfaceView.EGLConfigChooser {
        ...
        private void printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
            int[] attributes = {EGL10.EGL_BUFFER_SIZE, EGL10.EGL_ALPHA_SIZE,
                    EGL10.EGL_BLUE_SIZE, EGL10.EGL_GREEN_SIZE,
