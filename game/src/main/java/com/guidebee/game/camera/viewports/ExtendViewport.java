 ...
 * direction. The world is first scaled to fit within
 * the viewport, then the shorter dimension is lengthened to fill the viewport.
 * A maximum size can be specified to limit how much
 * the world is extended and black bars (letterboxing) are used for any
 * remaining space.
 *
 * @author Nathan Sweet
 */
public class ExtendViewport extends Viewport {
     ...
     * {@link com.guidebee.game.camera.OrthographicCamera} with no maximum world size.
     */
    ...
    public void update(int screenWidth, int screenHeight,
        ...
        if (viewportWidth < screenWidth) {
            ...
            if (maxWorldWidth > 0) lengthen = Math.min(lengthen,
                    maxWorldWidth - minWorldWidth);
            worldWidth += lengthen;
            ...
            if (maxWorldHeight > 0) lengthen = Math.min(lengthen,
                    maxWorldHeight - minWorldHeight);
