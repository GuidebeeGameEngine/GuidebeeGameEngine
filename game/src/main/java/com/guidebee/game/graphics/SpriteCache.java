 ...
 * textures are cached and given an ID, which can
 * later be used for drawing. The size, color, and texture region for each cached
 * image cannot be modified. This information is
 * stored in video memory and does not have to be sent to the GPU each time it
 * is drawn.<br>
 * <br>
 * To cache {@link Sprite sprites} or
 * {@link com.guidebee.game.graphics.Texture textures}, first call
 * {@link SpriteCache#beginCache()}, then call the
 * appropriate add method to define the images. To complete the cache, call
 * {@link SpriteCache#endCache()} and store the returned
 * cache ID.<br>
 * <br>
 * To draw with SpriteCache, first call {@link #begin()}, then call
 * {@link #draw(int)} with a cache ID. When SpriteCache drawing
 * is complete, call {@link #end()}.<br>
 * <br>
 * By default, SpriteCache draws using screen coordinates and uses an x-axis
 * pointing to the right, an y-axis pointing upwards and
 * the origin is the bottom left corner of the screen. The default
 * transformation and projection matrices can be changed. If the
 * screen is
 * {@link com.guidebee.game.ApplicationListener#resize(int, int) resized},
 * the SpriteCache's matrices must be updated. For example:<br>
 * <code>cache.getProjectionMatrix().setToOrtho2D(0, 0, GameEngine.graphics.getWidth(),
 * GameEngine.graphics.getHeight());</code><br>
 * <br>
 * Note that SpriteCache does not manage blending. You will need to enable blending
 * (<i>GameEngine.gl.glEnable(GL10.GL_BLEND);</i>) and
 ...
 * OpenGL resources a SpriteCache uses internally are
 * restored.<br>
 * <br>
 * SpriteCache is a reasonably heavyweight object. Typically only one instance
 * should be used for an entire application.<br>
 * <br>
 * SpriteCache works with OpenGL ES 1.x and 2.0. For 2.0, it uses its own
 * custom shader to draw.<br>
 * <br>
...
public class SpriteCache implements Disposable {
     ...
     *
     * @param size       The maximum number of images this cache can hold.
     *                   The memory required to hold the images is allocated up front.
    ...
    /**
     * Starts the redefinition of an existing cache, allowing the add
     * and {@link #endCache()} methods to be called. If this is not
     * the last cache created, it cannot have more entries added to it
     * than when it was first created. To do that, use
            ...
            if (cacheCount > cache.maxCount) {
                throw new GameEngineRuntimeException(
                        "If a cache is not the last created," +
                                " it cannot be redefined with more entries" +
    ...
    /**
     * Invalidates all cache IDs and resets the SpriteCache so new
     * caches can be added.
     */
    public void clear() {
        caches.clear();
     ...
     * elements, one for each of the attributes: x, y, color,
     * u, and v. If indexed geometry is used, each image should be specified
     * as 4 vertices, otherwise each image should be
     ...
     * Adds the specified texture to the cache.
     */
    public void add(Texture texture, float x, float y, int srcWidth,
                    int srcHeight, float u, float v, float u2, float v2,
                    float color) {
     ...
     * Adds the specified texture to the cache.
     */
    public void add(Texture texture, float x, float y, int srcX,
     ...
     * Adds the specified texture to the cache.
     */
    public void add(Texture texture, float x, float y, float width,
    ...
    public void add(Texture texture, float x, float y, float originX,
     ...
     * Adds the specified region to the cache.
     */
    public void add(TextureRegion region, float x, float y, float originX,
    ...
    public void add(Sprite sprite) {
        ...
        System.arraycopy(spriteVertices, 0, tempVertices, 0, 3 * Sprite.VERTEX_SIZE);
        // temp0,1,2=sprite0,1,2
        System.arraycopy(spriteVertices, 2 * Sprite.VERTEX_SIZE, tempVertices,
                3 * Sprite.VERTEX_SIZE, Sprite.VERTEX_SIZE); // temp3=sprite2
        System.arraycopy(spriteVertices, 3 * Sprite.VERTEX_SIZE, tempVertices,
                4 * Sprite.VERTEX_SIZE, Sprite.VERTEX_SIZE); // temp4=sprite3
        System.arraycopy(spriteVertices, 0, tempVertices,
                5 * Sprite.VERTEX_SIZE, Sprite.VERTEX_SIZE); // temp5=sprite0
    ...
    public void begin() {
        ...
        GameEngine.gl20.glDepthMask(false);
     ...
     * attribute is called "a_position", the texture
     * coordinates attribute is called called "a_texCoords", the color attribute
     * is called "a_color". The projection matrix is
     * uploaded via a mat4 uniform called "u_proj", the transform matrix is
     * uploaded via a uniform called "u_trans", the combined
     * transform and projection matrx is is uploaded via a mat4 uniform called
     * "u_projTrans". The texture sampler is passed via a
