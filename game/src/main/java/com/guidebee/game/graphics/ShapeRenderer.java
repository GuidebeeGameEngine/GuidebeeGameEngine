 ...
 * corner is used and units are specified in screen
 * pixels. This can be changed by configuring the projection matrix, usually
 * using the {@link com.guidebee.game.camera.Camera#combined} matrix. If the
 ...
 * used to rotate, scale and translate shapes in a
 * more flexible manner. The following example shows how to rotate a rectangle
 * around its center using the z-axis as the rotation
 ...
 * glScale and glRotate. The last transformation
 * specified will be the first that is applied to a shape (rotate then translate
 * in the above example).
 * <p>
 * The projection and transformation matrices are a state of the ShapeRenderer,
 * just like the color, and will be applied to all
...
public class ShapeRenderer implements Disposable {
    ...
    /**
     * Sets the projection matrix to be used for rendering. Usually this will
     * be set to {@link com.guidebee.game.camera.Camera#combined}.
     *
     * @param matrix
     */
     ...
     * type, the batch is flushed and the shape type is
     * changed automatically. This can increase the number of batch flushes if
     * care is not taken to draw the same type of shapes
     ...
     * to use the type specified. The call to this method
     * must be paired with a call to {@link #end()}.
     *
     * @see #setAutoShapeType(boolean)
     */
    public void begin(ShapeType type) {
        if (shapeType != null) throw new IllegalStateException("Call end()" +
                " before beginning a new shape batch.");
        shapeType = type;
        if (matrixDirty) {
    ...
    /**
     * Draws a point using {@link ShapeType#Point}, {@link ShapeType#Line}
     * or {@link ShapeType#Filled}.
     */
     ...
     */
    public void line(float x, float y, float z, float x2, float y2,
    ...
    public void curve(float x1, float y1, float cx1, float cy1, float cx2,
        ...
        // Algorithm from: http://www.antigrain.com/research/bezier_interpolatio
        // n/index.html#PAGE_BEZIER_INTERPOLATION
    ...
    /**
     * Draws a triangle in x/y plane using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}.
     */
    ...
    public void triangle(float x1, float y1, float x2, float y2,
                         float x3, float y3, Color col1, Color col2, Color col3) {
        check(ShapeType.Line, ShapeType.Filled, 6);
        if (shapeType == ShapeType.Line) {
    ...
    /**
     * Draws a rectangle in the x/y plane using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}.
     */
    ...
    /**
     * Draws a rectangle in the x/y plane using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}. The x and y specify the lower
     * left corner.
     *
     ...
     */
    public void rect(float x, float y, float width, float height, Color col1,
     ...
     * Draws a rectangle in the x/y plane using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}. The x and y specify the lower
     * left corner. The originX and originY specify the point about
    ...
    public void rect(float x, float y, float originX, float originY,
                     ...
                     float degrees) {
        rect(x, y, originX, originY, width, height, scaleX, scaleY,
                degrees, color, color, color, color);
    }

    /**
     * Draws a rectangle in the x/y plane using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}. The x and y specify the lower
     * left corner. The originX and originY specify the point about which
    ...
    public void rect(float x, float y, float originX, float originY,
                     float width, float height, float scaleX, float scaleY,
                     float degrees, Color col1, Color col2,
    ...
    /**
     * Draws a line using a rotated rectangle, where with one edge is
     * centered at x1, y1 and the opposite edge centered at x2, y2.
     */
    ...
    /**
     * Draws two crossed lines using {@link ShapeType#Line}
     * or {@link ShapeType#Filled}.
     */
    ...
    /**
     * Calls {@link #arc(float, float, float, float, float, int)}
     ...
     */
    public void arc(float x, float y, float radius, float start,
                    float degrees, int segments) {
    ...
    /**
     * Calls {@link #ellipse(float, float, float, float, int)}
     * by estimating the number of segments needed for a smooth ellipse.
     */
    public void ellipse(float x, float y, float width, float height) {
        ellipse(x, y, width, height, Math.max(1,
                (int) (12 * (float) Math.cbrt(Math.max(width * 0.5f, height * 0.5f)))));
    }
    ...
    /**
     * Calls {@link #cone(float, float, float, float, float, int)}
    ...
    /**
     * Draws a polyline in the x/y plane using {@link ShapeType#Line}.
    ...
    private void check(ShapeType preferred, ShapeType other, int newVertices) {
        ...
        if (shapeType != preferred && shapeType != other) {
            ...
            if (!autoShapeType) {
                if (other == null)
                    throw new IllegalStateException("Must call begin(ShapeType."
                            + preferred + ").");
                else
                    throw new IllegalStateException("Must call begin(ShapeType."
                            + preferred + ") or begin(ShapeType." + other + ").");
            }
            end();
            begin(preferred);
        } else if (matrixDirty) {
