...
import com.guidebee.game.graphics.TextureAtlas.TextureAtlasData.Page;
import com.guidebee.game.graphics.TextureAtlas.TextureAtlasData.Region;
 ...
 * <br>
 * A TextureAtlas must be disposed to free up the resources consumed by
 * the backing textures.
 *
 * @author Nathan Sweet
 */
public class TextureAtlas implements Disposable {
    ...
    public static class TextureAtlasData {
        ...
        public static class Page {
            ...
            public Page(FileHandle handle, float width, float height,
                        boolean useMipMaps, Pixmap.Format format,
        ...
        public TextureAtlasData(FileHandle packFile,
                                FileHandle imagesDir, boolean flip) {
            BufferedReader reader
                    = new BufferedReader(new InputStreamReader(packFile.read()), 64);
            try {
                Page pageImage = null;
                while (true) {
                    ...
                    if (line.trim().length() == 0)
                        ...
                        if (readTuple(reader) == 2) { // size is only optional
                            // for an atlas packed with an old TexturePacker.
                        ...
                        pageImage = new Page(file, width, height, min.isMipMap(), format,
                ...
                throw new GameEngineRuntimeException("Error reading pack file: "
                        + packFile, ex);
            } finally {
     ...
     * {@link com.guidebee.game.Files.FileType#Internal},
     * using the parent directory of the pack file to find the page
     * images.
     */
    public TextureAtlas(String internalPackFile) {
        this(GameEngine.files.internal(internalPackFile));
    }
    ...
    /**
     * @param flip If true, all regions loaded will be flipped for use
     *             with a perspective where 0,0 is the upper left corner.
    ...
    private void load(TextureAtlasData data) {
        ...
        for (Region region : data.regions) {
            ...
            AtlasRegion atlasRegion = new AtlasRegion(pageToTexture.get(region.page),
    ...
    public AtlasRegion addRegion(String name, TextureRegion textureRegion) {
        return addRegion(name, textureRegion.texture, textureRegion.getRegionX(),
     ...
     * string comparison to find the region, so the result
     ...
     * This method uses string comparison to find the region, so
     ...
     * largest {@link AtlasRegion#index index}. This method
     * uses string comparison to find the regions, so the result should be
    ...
    /**
     * Returns all regions in the atlas as sprites. This method creates
     * a new sprite for each region, so the result should be
     ...
     * If whitespace was stripped from the region when it was
     * packed, the sprite is automatically positioned as if whitespace
     * had not been stripped. This method uses string comparison to
     * find the region and constructs a new sprite, so the result should
    ...
    /**
     * Returns the first region found with the specified name and index as
     * a sprite. This method uses string comparison to find the
     * region and constructs a new sprite, so the result should be cached
     ...
     * smallest to largest {@link AtlasRegion#index index}. This
     * method uses string comparison to find the regions and constructs
     * new sprites, so the result should be cached rather than
     ...
     * ninepatch splits. This method uses string comparison to find
     * the region and constructs a new ninepatch, so the result should
     * be cached rather than calling this method multiple times.
     *
     ...
     * This releases all the textures backing all TextureRegions
    ...
    static String readValue(BufferedReader reader) throws IOException {
        ...
        if (colon == -1) throw new GameEngineRuntimeException("Invalid line: " + line);
    ...
    /**
     * Describes the region of a packed image and provides information about
     * the original image before it was packed.
     */
    static public class AtlasRegion extends TextureRegion {
         ...
         * it is stored as the index and is not considered as
         * part of the sprite's name. This is useful for keeping animation frames
         * in order.
         *
         ...
         * Underscores denote special instructions to the texture
         * packer.
         */
        public String name;
        ...
        /**
         * The offset from the bottom of the original image to the bottom of
         * the packed image, after whitespace was removed for
         * packing.
         */
        public float offsetY;
        ...
        /**
         * The width of the image, before whitespace was removed and
         * rotation was applied for packing.
         */
        public int originalWidth;
        ...
        @Override
        /** Flips the region, adjusting the offset so the image appears to be flip
         *  as if no whitespace has been removed for packing. */
         ...
         * then it returns the packedHeight, otherwise it
         ...
         * then it returns the packedWidth, otherwise it
    ...
    static public class AtlasSprite extends Sprite {
        ...
        public void rotate90(boolean clockwise) {
            ...
            if (clockwise) {
                ...
                region.offsetY = region.originalHeight * heightRatio - oldOffsetX
