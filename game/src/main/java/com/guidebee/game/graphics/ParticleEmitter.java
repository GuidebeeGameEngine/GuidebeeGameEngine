...
public class ParticleEmitter {
    ...
    public void update(float delta) {
        ...
        if (delayTimer < delay) {
            ...
            if (!done) {
                ...
                float emissionTime = emission
                        + emissionDiff * emissionValue.getScale(durationTimer / (float) duration);
                if (emissionTime > 0) {
    ...
    public void draw(Batch batch) {
        ...
        if (additive || premultipliedAlpha) batch.setBlendFunction(IGL20.GL_SRC_ALPHA,
                IGL20.GL_ONE_MINUS_SRC_ALPHA);
    ...
    private boolean updateParticle(Particle particle, float delta, int deltaMillis) {
        ...
        if ((updateFlags & UPDATE_VELOCITY) != 0) {
            float velocity = (particle.velocity
                    + particle.velocityDiff * velocityValue.getScale(percent)) * delta;
            ...
            if ((updateFlags & UPDATE_ANGLE) != 0) {
                ...
                if ((updateFlags & UPDATE_ROTATION) != 0) {
                    float rotation = particle.rotation
                            + particle.rotationDiff * rotationValue.getScale(percent);
                    if (aligned) rotation += angle;
                    particle.setRotation(rotation);
                }
            } else {
            ...
            if ((updateFlags & UPDATE_GRAVITY) != 0)
                velocityY += (particle.gravity
                        + particle.gravityDiff * gravityValue.getScale(percent)) * delta;
    ...
    /**
     * Ignores the {@link #setContinuous(boolean) continuous}
