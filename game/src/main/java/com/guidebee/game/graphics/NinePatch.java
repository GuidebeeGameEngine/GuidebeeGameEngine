 ...
 * be set as a hint on how to inset content on top
 * of the ninepatch (by default the eight "edge" textures of the nine-patch define
 * the padding). When drawn the eight "edge"
 ...
 * ".9.png" texture. That is, the textures given to
 * this class should <em>not</em> include the meta-data pixels from a ".9.png"
 * that describe the layout of the ninepatch over the
 * interior of the graphic. That information should be passed into the
 * constructor either implicitly as the size of the individual
 * patch textures, or via the <code>left, right, top, bottom</code> parameters
 * to {@link #NinePatch(com.guidebee.game.graphics.Texture, int, int, int, int)}
...
public class NinePatch {
    ...
    private float leftWidth, rightWidth, middleWidth, middleHeight,
     ...
     * The subsequent parameters define the 4 lines that will
    ...
    /**
     * Create a ninepatch by cutting up the given texture region into nine
     * patches. The subsequent parameters define the 4 lines
    ...
    public NinePatch(TextureRegion region, int left, int right, int top, int bottom) {
        ...
        if (top > 0) {
            if (left > 0) patches[TOP_LEFT] = new TextureRegion(region, 0, 0,
                    left, top);
            if (middleWidth > 0) patches[TOP_CENTER] = new TextureRegion(region,
                    left, 0, middleWidth, top);
            if (right > 0) patches[TOP_RIGHT] = new TextureRegion(region, left
                    + middleWidth, 0, right, top);
        }
        if (middleHeight > 0) {
            if (left > 0) patches[MIDDLE_LEFT] = new TextureRegion(region, 0, top,
                    left, middleHeight);
            if (middleWidth > 0)
                patches[MIDDLE_CENTER] = new TextureRegion(region, left, top,
                        middleWidth, middleHeight);
            if (right > 0)
                patches[MIDDLE_RIGHT] = new TextureRegion(region, left + middleWidth,
                        top, right, middleHeight);
        }
        if (bottom > 0) {
            if (left > 0) patches[BOTTOM_LEFT] = new TextureRegion(region, 0,
                    top + middleHeight, left, bottom);
            if (middleWidth > 0)
                patches[BOTTOM_CENTER] = new TextureRegion(region, left,
                        top + middleHeight, middleWidth, bottom);
            if (right > 0)
                patches[BOTTOM_RIGHT] = new TextureRegion(region, left + middleWidth,
                        top + middleHeight, right, bottom);
        }
     ...
     * patches must be consistently sized (e.g., any left
     * edge textures must have the same width, etc). Patches may be <code>null</code>.
    ...
    public NinePatch(TextureRegion... patches) {
        ...
        if ((patches[TOP_LEFT] != null && patches[TOP_LEFT].getRegionWidth() != leftWidth)
                ...
                && patches[MIDDLE_LEFT].getRegionWidth() != leftWidth)
                || (patches[BOTTOM_LEFT] != null
                && patches[BOTTOM_LEFT].getRegionWidth() != leftWidth)) {
            throw new GameEngineRuntimeException("Left side patches must have the same width");
        }
        ...
        if ((patches[TOP_RIGHT] != null
                ...
                || (patches[MIDDLE_RIGHT] != null
                && patches[MIDDLE_RIGHT].getRegionWidth() != rightWidth)
                || (patches[BOTTOM_RIGHT] != null
                && patches[BOTTOM_RIGHT].getRegionWidth() != rightWidth)) {
            throw new GameEngineRuntimeException("Right side patches must have the same width");
        }

        float bottomHeight = getBottomHeight();
        if ((patches[BOTTOM_LEFT] != null
                && patches[BOTTOM_LEFT].getRegionHeight() != bottomHeight)
                || (patches[BOTTOM_CENTER] != null
                && patches[BOTTOM_CENTER].getRegionHeight() != bottomHeight)
                || (patches[BOTTOM_RIGHT] != null
                && patches[BOTTOM_RIGHT].getRegionHeight() != bottomHeight)) {
            throw new GameEngineRuntimeException("Bottom side patches must have the same height");
        }

        float topHeight = getTopHeight();
        if ((patches[TOP_LEFT] != null
                && patches[TOP_LEFT].getRegionHeight() != topHeight)
                || (patches[TOP_CENTER] != null
                && patches[TOP_CENTER].getRegionHeight() != topHeight)
                || (patches[TOP_RIGHT] != null
                && patches[TOP_RIGHT].getRegionHeight() != topHeight)) {
            throw new GameEngineRuntimeException("Top side patches must have the same height");
        }
    }
    ...
    public NinePatch(NinePatch ninePatch, Color color) {
        ...
        System.arraycopy(ninePatch.vertices, 0, vertices,
                0, ninePatch.vertices.length);
        idx = ninePatch.idx;
    ...
    public void draw(Batch batch, float x, float y, float width, float height) {
        ...
        if (bottomCenter != -1) set(bottomCenter, centerColumnX, y,
                rightColumnX - centerColumnX, middleRowY - y, c);
        if (bottomRight != -1) set(bottomRight, rightColumnX, y,
                x + width - rightColumnX, middleRowY - y, c);
        if (middleLeft != -1) set(middleLeft, x, middleRowY, centerColumnX - x,
                topRowY - middleRowY, c);
        if (middleCenter != -1)
            set(middleCenter, centerColumnX, middleRowY, rightColumnX - centerColumnX,
                    topRowY - middleRowY, c);
        if (middleRight != -1)
            set(middleRight, rightColumnX, middleRowY, x + width - rightColumnX,
                    topRowY - middleRowY, c);
        if (topLeft != -1) set(topLeft, x, topRowY, centerColumnX - x,
                y + height - topRowY, c);
        if (topCenter != -1)
            set(topCenter, centerColumnX, topRowY, rightColumnX - centerColumnX,
                    y + height - topRowY, c);
        if (topRight != -1) set(topRight, rightColumnX, topRowY,
                x + width - rightColumnX, y + height - topRowY, c);
    ...
    /**
     * Set the width of the middle column of the patch. At render time, this
     * is implicitly the requested render-width of the entire
     * nine patch, minus the left and right width. This value is only used
     * for computing the {@link #getTotalWidth() default total
     * width}.
     */
    ...
    /**
     * Set the height of the middle row of the patch. At render time, this
     * is implicitly the requested render-height of the entire
     * nine patch, minus the top and bottom height. This value is only
     * used for computing the {@link #getTotalHeight() default
     * total height}.
     */
    ...
    /**
     * Set the padding for content inside this ninepatch. By default
     * the padding is set to match the exterior of the ninepatch, so
