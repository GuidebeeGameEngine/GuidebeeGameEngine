 ...
 * (region). The class will batch the drawing commands
 * and optimize them for processing by the GPU.
 * <p/>
 * To draw something with a PolygonSpriteBatch one has to first call the
 * {@link PolygonSpriteBatch#begin()} method which will
 * setup appropriate render states. When you are done with drawing you have to
 * call {@link PolygonSpriteBatch#end()} which will
 * actually draw the things you specified.
 * <p/>
 * All drawing commands of the PolygonSpriteBatch operate in screen coordinates.
 * The screen coordinate system has an x-axis
 * pointing to the right, an y-axis pointing upwards and the origin is in the
 * lower left corner of the screen. You can also
 ...
 * OpenGL resources a PolygonSpriteBatch uses internally
 * get invalidated. A context is lost when a user switches to another application
 * or receives an incoming call on Android. A
 * SpritPolygonSpriteBatcheBatch will be automatically reloaded after the OpenGL
 * context is restored.
 * <p/>
 ...
 * context it will use its own custom shader to draw
...
public class PolygonSpriteBatch implements Batch {
    ...
    /**
     * Constructs a new PolygonSpriteBatch with a size of 2000,
     * the default shader, and one buffer.
     *
     ...
     * orthographic projection with y-axis point upwards,
     * x-axis point to the right and the origin being in the bottom left
     * corner of the screen. The projection will be pixel perfect
     * with respect to the current screen resolution.
     * <p/>
     * The defaultShader specifies the shader to use. Note that the names
     * for uniforms for this default shader are different than
     * the ones expect for shaders set with {@link #setShader(ShaderProgram)}.
     * See {@link SpriteBatch#createDefaultShader()}.
     *
     * @param size          The max number of vertices and number of triangles
     *                      in a single batch. Max of 10920.
     * @param defaultShader The default shader to use. This is not owned by
     *                      PolygonSpriteBatch and must be disposed separately.
     */
    public PolygonSpriteBatch(int size, ShaderProgram defaultShader) {
        // 32767 is max index, so 32767 / 3 - (32767 / 3 % 3) = 10920.
        if (size > 10920)
            throw new IllegalArgumentException("Can't have more than 10920 triangles per batch: "
                    + size);

        mesh = new Mesh(VertexDataType.VertexArray, false, size, size * 3,
    ...
    /**
     * Draws a polygon region with the bottom left corner at x,y and
     * stretching the region to cover the given width and height.
     */
    public void draw(PolygonRegion region, float x, float y, float width,
                     float height) {
        if (!drawing)
            throw new IllegalStateException("PolygonSpriteBatch.begin must be called before draw.");
     ...
     * The polygon region is offset by originX, originY relative to the origin.
     * Scale specifies the scaling factor by which the
     * polygon region should be scaled around originX, originY. Rotation
     * specifies the angle of counter clockwise rotation of the
     * rectangle around originX, originY.
     */
    public void draw(PolygonRegion region, float x, float y, float originX,
    ...
    /**
     * Draws the polygon using the given vertices and triangles. Each vertices
     * must be made up of 5 elements in this order: x, y,
     * color, u, v.
     */
    public void draw(Texture texture, float[] polygonVertices, int verticesOffset,
                     int verticesCount, short[] polygonTriangles,
        ...
        System.arraycopy(polygonVertices, verticesOffset,
                vertices, vertexIndex, verticesCount);
    ...
    public void draw(Texture texture, float x, float y, float originX,
        ...
        if (texture != lastTexture)
            switchTexture(texture);
                ...
                || vertexIndex + Sprite.SPRITE_SIZE > vertices.length) //
            flush();
    ...
    public void draw(Texture texture, float x, float y, int srcX,
    ...
    public void draw(Texture texture, float[] spriteVertices,
        ...
        final int triangleCount = count / Sprite.SPRITE_SIZE * 6;
        if (texture != lastTexture)
            switchTexture(texture);
        ...
        final int vertexIndex = this.vertexIndex;
        int triangleIndex = this.triangleIndex;
        short vertex = (short) (vertexIndex / Sprite.VERTEX_SIZE);
        for (int n = triangleIndex + triangleCount; triangleIndex < n;
    ...
    public void draw(TextureRegion region, float x, float y, float
            width, float height) {
        if (!drawing)
            throw new IllegalStateException("PolygonSpriteBatch.begin must be called before draw.");
