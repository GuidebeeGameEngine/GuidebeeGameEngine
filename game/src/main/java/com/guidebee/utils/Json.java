...
import com.guidebee.utils.collections.ArrayMap;
 ...
 * https://github.com/libgameengine/libgameengine/wiki/Reading-%26-writing-JSON
 *
 * @author Nathan Sweet
 */
public class Json {
    ...
    private final ObjectMap<Class, ObjectMap<String, FieldMetadata>>
            typeToFields = new ObjectMap();
    ...
    /**
     * Sets the name of the JSON field to store the Java class name or
     * class tag when required to avoid ambiguity during
     * deserialization. Set to null to never output this information,
    ...
    public void setElementType(Class type, String fieldName, Class elementType) {
        ...
        if (metadata == null)
            throw new SerializationException("Field not found: " + fieldName
                    + " (" + type.getName() + ")");
     ...
     * @param elementType May be null if the type is unknown.
     */
    public void toJson(Object object, Class knownType, Class elementType,
                       Writer writer) {
        setWriter(writer);
        try {
    ...
    /**
     * Sets the writer where JSON output will go. This is only necessary
    ...
    public void writeFields(Object object) {
        ...
        for (FieldMetadata metadata : new Values<FieldMetadata>(fields)) {
            ...
            try {
                ...
                if (debug) System.out.println("Writing field: " + field.getName()
                        + " (" + type.getName() + ")");
    ...
    private Object[] getDefaultValues(Class type) {
        ...
        for (FieldMetadata metadata : fields.values()) {
            ...
            try {
                ...
                throw new SerializationException("Error accessing field: "
    ...
    public void writeField(Object object, String fieldName, String jsonName, Class elementType) {
        ...
        try {
            if (debug) System.out.println("Writing field: " + field.getName() + " (" + type
                    .getName() + ")");
     ...
     * the object if it differs from the specified
     * known type.
     *
     * @param value     May be null.
     * @param knownType May be null if the type is unknown.
    ...
    /**
     * Writes the value, writing the class of the object if it differs from the
     * specified known type. The specified element type is
    ...
    public void writeValue(Object value, Class knownType, Class elementType) {
        try {
            ...
            if ((knownType != null && knownType.isPrimitive()) || knownType
                    == String.class || knownType == Integer.class
                    || knownType == Boolean.class || knownType == Float.class
                    || knownType == Long.class || knownType == Double.class
                    || knownType == Short.class || knownType == Byte.class
            ...
            if (actualType.isPrimitive() || actualType == String.class
                    || actualType == Integer.class || actualType == Boolean.class
                    || actualType == Float.class || actualType == Long.class
                    || actualType == Double.class || actualType == Short.class
            ...
            if (value instanceof Array) {
                if (knownType != null && actualType != knownType
                        && actualType != Array.class)
                    throw new SerializationException("Serialization of an Array other than " +
                            "the known type is not supported.\n"
            ...
            if (value instanceof Collection) {
                if (knownType != null && actualType != knownType
                    ...
                    throw new SerializationException("Serialization of a Collection " +
                            "other than the known type is not supported.\n"
    ...
    public void readField(Object object, String name, Class elementType,
                          JsonValue jsonData) {
        readField(object, name, name, elementType, jsonData);
    }

    public void readField(Object object, String fieldName, String jsonName,
                          JsonValue jsonData) {
     ...
     */
    public void readField(Object object, String fieldName, String jsonName,
                          Class elementType, JsonValue jsonMap) {
        Class type = object.getClass();
        ObjectMap<String, FieldMetadata> fields = typeToFields.get(type);
        if (fields == null) fields = cacheFields(type);
        FieldMetadata metadata = fields.get(fieldName);
        if (metadata == null)
            throw new SerializationException("Field not found: "
        ...
        for (JsonValue child = jsonMap.child; child != null; child = child.next) {
            ...
            if (metadata == null) {
                if (ignoreUnknownFields) {
                    if (debug)
                        System.out.println("Ignoring unknown field: "
                                + child.name() + " (" + type.getName() + ")");
                    continue;
                } else
                    throw new SerializationException("Field not found: "
                            + child.name() + " (" + type.getName() + ")");
     ...
     * @return May be null.
     */
    public <T> T readValue(String name, Class<T> type, T defaultValue,
                           JsonValue jsonMap) {
     ...
     * @return May be null.
     */
    public <T> T readValue(String name, Class<T> type, Class elementType,
                           JsonValue jsonMap) {
     ...
     */
    public <T> T readValue(String name, Class<T> type, Class elementType,
                           T defaultValue, JsonValue jsonMap) {
        JsonValue jsonValue = jsonMap.get(name);
        if (jsonValue == null) return defaultValue;
    ...
    public <T> T readValue(Class<T> type, Class elementType, JsonValue jsonData) {
        ...
        if (jsonData.isArray()) {
            if ((type == null || type == Object.class)
                    || ClassReflection.isAssignableFrom(Array.class, type)) {
                Array newArray = (type == null ||
                        type == Object.class) ? new Array() : (Array) newInstance(type);
                for (JsonValue child = jsonData.child; child != null; child = child.next)
                    newArray.add(readValue(elementType, null, child));
                return (T) newArray;
            }
            if (ClassReflection.isAssignableFrom(List.class, type)) {
                List newArray = (type == null
                        || type.isInterface()) ? new ArrayList() : (List) newInstance(type);
                for (JsonValue child = jsonData.child; child != null; child = child.next)
                    newArray.add(readValue(elementType, null, child));
                return (T) newArray;
            }
