...
//[------------------------------ MAIN CLASS ----------------------------------]
/**
 * This is a near duplicate of {@link TimSort}, modified for use with arrays
 * of objects that implement {@link Comparable}, instead
 * of using explicit comparators.
 * <p>
 * <p>
 * If you are using an optimizing VM, you may find that ComparableTimSort
 * offers no performance benefit over TimSort in
 * conjunction with a comparator that simply returns {@code ((Comparable)first).
 * compareTo(Second)}. If this is the case, you are
 * better off deleting ComparableTimSort to eliminate the code duplication.
 * (See Arrays.java for details.)
 */
class ComparableTimSort {
     ...
     * C implementation, but 32 was empirically determined to work
     * better in this implementation. In the unlikely event that you set
     * this constant to be a number that's not a power of two,
     ...
     * to MIN_GALLOP. The mergeLo and mergeHi methods nudge it
     ...
     * storage when sorting smaller arrays. This change was required
     * for performance.
     */
     ...
     * base[i] and extends for len[i] elements. It's always true
    ...
    /**
     * Asserts have been placed in if-statements for performace. To enable
     * them, set this field to true and enable them in VM with
    ...
    private ComparableTimSort(Object[] a) {
         ...
         * requirements are described in listsort.txt.
		 * The C version always uses the same stack length (85), but this was measured
		 * to be too expensive when sorting "mid-sized"
		 * arrays (e.g., 100 elements) in Java. Therefore, we use smaller
		 * (but sufficiently large) stack lengths for smaller arrays.
		 * The "magic numbers" in the computation below must be changed if MIN_MERGE
		 * is decreased. See the MIN_MERGE declaration
	 ...
	 * The next two methods (which are package private and static) constitute the
	  * entire API of this class. Each of these methods
	 * obeys the contract of the public method with the same signature
	 * in java.util.Arrays.
	 */
     ...
     * numbers of elements. It requires O(n log n) compares, but O(n^2)
     * data movement (worst case).
     * <p>
     * If the initial part of the specified range is already sorted, this
     * method can take advantage of it: the method assumes that
    ...
    private static void binarySort(Object[] a, int lo, int hi, int start) {
        ...
        for (; start < hi; start++) {
			 ...
			 * that if there are elements equal to pivot, left points to
			  * the first slot after them -- that's why this sort is stable.
    ...
    /**
     * Returns the length of the run beginning at the specified position
     * in the specified array and reverses the run if it is
     ...
     * Natural runs shorter than this will be
    ...
    /**
     * Merges all runs on the stack until only one remains. This method
     * is called once, to complete the sort.
     */
    ...
    private void mergeAt(int i) {
		 ...
		 * now, also slide over the last run (which isn't involved
		...
		/*
		 * Find where the first element of run2 goes in run1. Prior
		 * elements in run1 can be ignored (because they're already in
		 * place).
		 */
     ...
     *             The closer hint is to the result, the faster this method
     *             will run.
     * @return the int k, 0 <= k <= n such that a[b + k - 1] < key <= a[b + k],
     * pretending that a[b - 1] is minus infinity and a[b
     * + n] is infinity. In other words, key belongs at index b + k; or in other
     * words, the first k elements of a should
    ...
    private static int gallopLeft(Comparable<Object> key, Object[] a,
		 ...
		 * Do a binary search, with invariant a[base + lastOfs - 1]
		 * < key <= a[base + ofs].
		 */
        lastOfs++;
        while (lastOfs < ofs) {
        ...
        if (DEBUG) assert lastOfs == ofs; // so a[base + ofs - 1]
        // < key <= a[base + ofs]
        return ofs;
    }
     ...
     * gallopRight returns the index after the
     ...
     *             hint is to the result, the faster this method
     *             will run.
     * @return the int k, 0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
     */
    private static int gallopRight(Comparable<Object> key, Object[] a, int base,
		 ...
		 * right of lastOfs but no farther right than ofs.
     ...
     * the first run must be greater than the first
     * element of the second run (a[base1] > a[base2]), and the last element of the
     * first run (a[base1 + len1-1]) must be greater
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        ...
        while (true) {
			 ...
			 * One run is winning so consistently that galloping may be a huge win.
			  * So try that, and continue galloping until (if
    ...
    private void mergeHi(int base1, int len1, int base2, int len2) {
        ...
        while (true) {
            ...
            do {
                ...
                count2 = len2 - gallopLeft((Comparable) a[cursor1], tmp, 0,
                        len2, len2 - 1);
                if (count2 != 0) {
                    dest -= count2;
                    cursor2 -= count2;
    ...
    /**
     * Ensures that the external array tmp has at least the specified number of elements,
     * increasing its size if necessary. The
