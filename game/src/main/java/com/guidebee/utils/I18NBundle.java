...
/**
 * A {@code I18NBundle} provides {@code Locale}-specific resources loaded
 * from property files. A bundle contains a number of named
 * resources, whose names and values are {@code Strings}. A bundle may have
 * a parent bundle, and when a resource is not found in a
 * bundle, the parent bundle is searched for the resource. If the fallback
 * mechanism reaches the base bundle and still can't find
 ...
 * This base bundle acts as the root and is the last
 ...
 * differences between a language and the language of
 ...
 * the same language. Only the differences between a
 * country and the country of the language bundle need to be handled by a
 * country-specific {@code I18NBundle}.</li>
 * <li>The third level contains changes that don't have a geographic reason
 * (e.g. changes that where made at some point in time
 * like {@code PREEURO} where the currency of come countries changed.
 * The country bundle would return the current currency (Euro)
 * and the {@code PREEURO} variant bundle would return the old currency
 * (e.g. DM for Germany).</li>
 * </ul>
 * <p/>
 ...
 * <li>BaseName_de_DE_PREEURO (bundle with Germany specific resources in
 * german of the time before the Euro)
 * <li>BaseName_fr_FR_PREEURO (bundle with France specific resources in
 * french of the time before the Euro)
 * </ul>
 * <p/>
 * It's also possible to create variants for languages or countries. This
 * can be done by just skipping the country or language
 * abbreviation: BaseName_us__POSIX or BaseName__DE_PREEURO. But it's not
 * allowed to circumvent both language and country:
...
public class I18NBundle {
    ...
    /**
     * The parent of this {@code I18NBundle} that is used if this bundle
    ...
    /**
     * Returns the flag indicating whether to use the simplified message
     * pattern syntax (default is false). This flag is always assumed to
     ...
     * The flag must be set before calling the factory methods {@code createBundle}.
     ...
     * @return a bundle for the given base file handle and the default
     * locale
     * @throws NullPointerException     if <code>baseFileHandle</code>
     *                                  is <code>null</code>
     * @throws MissingResourceException if no bundle for the specified
     *                                  base file handle can be found
     */
    ...
    /**
     * Creates a new bundle using the specified <code>baseFileHandle</code>
     * and <code>locale</code>; the default encoding "UTF-8"
     * is used.
     *
     * @param baseFileHandle the file handle to the base of the bundle
    ...
    /**
     * Creates a new bundle using the specified <code>baseFileHandle</code>
     * and <code>encoding</code>; the default locale is used.
     *
     * @param baseFileHandle the file handle to the base of the bundle
     ...
     * @throws MissingResourceException if no bundle for the specified base
     *                                  file handle can be found
     */
    public static I18NBundle createBundle(FileHandle baseFileHandle,
                                          String encoding) {
     ...
     * @return a bundle for the given base file handle and locale
     * @throws NullPointerException     if <code>baseFileHandle</code>,
     *                                  <code>locale</code> or <code>encoding</code> is
     *                                  <code>null</code>
     * @throws MissingResourceException if no bundle for the specified
     *                                  base file handle can be found
     */
    ...
    private static I18NBundle createBundleImpl(FileHandle baseFileHandle,
                                               Locale locale, String encoding) {
        ...
        do {
            ...
            if (bundle != null) {
                ...
                boolean isBaseBundle = bundleLocale.equals(Locale.US);
        ...
        if (bundle == null) {
            if (baseBundle == null) {
                // No bundle found
                throw new MissingResourceException("Can't find bundle for base file handle "
                        + baseFileHandle.path() + ", locale "
    ...
    /**
     * Returns a <code>List</code> of <code>Locale</code>s as candidate locales for the
     * given <code>locale</code>. This method is
     * called by the <code>createBundle</code> factory method each time the factory method
     * tries finding a resource bundle for a
     * target <code>Locale</code>.
     * <p/>
     * <p/>
     * The sequence of the candidate locales also corresponds to the runtime resource
     * lookup path (also known as the <I>parent
     * chain</I>), if the corresponding resource bundles for the candidate locales exist
     * and their parents are not defined by
     * loaded resource bundles themselves. The last element of the list is always the
     * {@linkplain Locale#ROOT root locale}, meaning
     * that the base bundle is the terminal of the parent chain.
     * <p/>
     * <p/>
     * If the given locale is equal to <code>Locale.ROOT</code> (the root locale),
     * a <code>List</code> containing only the root
     * <code>Locale</code> is returned. In this case, the <code>createBundle</code>
     * factory method loads only the base bundle as
     * the resulting resource bundle.
     * <p/>
     * <p/>
     * This implementation returns a <code>List</code> containing <code>Locale</code>s
     ...
     * </pre>
     * <p/>
     * where <code>language</code>, <code>country</code> and <code>variant</code> are
     * the language, country and variant values of
     * the given <code>locale</code>, respectively. Locales where the final component
     * values are empty strings are omitted.
     * <p/>
     * <p/>
     * For example, if the given base name is "Messages" and the given <code>locale</code> is
     * <code>Locale("ja",&nbsp;"",&nbsp;"XX")</code>, then a <code>List</code>
     * of <code>Locale</code>s:
     * <p/>
     * <pre>
     *     Locale("ja", "", "XX")
     *     Locale("ja")
     ...
     * </pre>
     * <p/>
     * is returned. And if the resource bundles for the "ja" and "" <code>Locale</code>s
     * are found, then the runtime resource
     ...
     * bundle searches by the <code>createBundle</code>
     * factory method. This method is called from the factory method every time
     * when no resulting bundle has been found for
     * <code>baseFileHandler</code> and <code>locale</code>, where locale is
     * either the parameter for <code>createBundle</code> or
     * the previous fallback locale returned by this method.
     * <p/>
     * <p/>
     * This method returns the {@linkplain Locale#getDefault() default
     * <code>Locale</code>} if the given <code>locale</code> isn't
     * the default one. Otherwise, <code>null</code> is returned.
     *
     * @param locale the <code>Locale</code> for which <code>createBundle</code>
     *               has been unable to find any resource bundles
    ...
    private static I18NBundle loadBundleChain(FileHandle baseFileHandle, String encoding,
        ...
        if (candidateIndex != candidateLocales.size() - 1) {
            ...
            parent = loadBundleChain(baseFileHandle, encoding, candidateLocales,
                    candidateIndex + 1, baseBundle);
        } else if (baseBundle != null && targetLocale.equals(Locale.US)) {
            return baseBundle;
        }
    ...
    private static I18NBundle loadBundle(FileHandle baseFileHandle, String encoding,
                                         Locale targetLocale) {
        I18NBundle bundle = null;
     ...
     * <p/>
     * <pre>
     * baseFileHandle.name() + &quot;_&quot; + language + &quot;_&quot; + country
     * + &quot;_&quot; + variant + &quot;.properties&quot;
     * </pre>
     * <p/>
     * where <code>language</code>, <code>country</code> and <code>variant</code>
     * are the language, country and variant values of
     * <code>locale</code>, respectively. Final component values that are empty
     * Strings are omitted along with the preceding '_'.
     * If all of the values are empty strings, then <code>baseFileHandle.name()</code>
     * is returned with ".properties" appended.
     *
     * @param baseFileHandle the file handle to the base of the bundle
     ...
     * to <code>createBundle()</code> to determine whether
     * the resource bundle returned really corresponds to the requested locale
     * or is a fallback.
     *
     ...
     * parent after replacing the given arguments if they
     * occur.
     *
     * @param key  the key for the desired string
