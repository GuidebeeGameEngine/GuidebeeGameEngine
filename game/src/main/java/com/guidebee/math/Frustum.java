 ...
 *
 * @see com.guidebee.game.camera.Camera#frustum
 */
public class Frustum {
     ...
     */
    public final Vector3[] planePoints = {new Vector3(), new Vector3(),
     ...
     * projection and view matrix, e.g. from an
     * {@link com.guidebee.game.camera.OrthographicCamera} or
     * {@link com.guidebee.game.camera.PerspectiveCamera}.
     *
     ...
     * @return Whether the sphere is in the frustum
     */
    public boolean sphereInFrustumWithoutNearFar(float x, float y,
                                                 float z, float radius) {
        for (int i = 2; i < 6; i++)
            if ((planes[i].normal.x * x + planes[i].normal.y * y
                    + planes[i].normal.z * z) < (-radius - planes[i].d))
                return false;
        return true;
    }
     ...
     * {@link com.guidebee.math.collision.BoundingBox} is in the frustum.
     *
    ...
    public boolean boundsInFrustum(BoundingBox bounds) {
        ...
        for (int i = 0, len2 = planes.length; i < len2; i++) {
            ...
            for (int j = 0; j < len; j++)
                if (planes[i].testPoint(corners[j]) == Plane.PlaneSide.Back) out++;
    ...
    public boolean boundsInFrustum(float x, float y, float z, float halfWidth,
        ...
        for (int i = 0, len2 = planes.length; i < len2; i++) {
            if (planes[i].testPoint(x + halfWidth, y + halfHeight, z + halfDepth)
                    != Plane.PlaneSide.Back) continue;
            if (planes[i].testPoint(x + halfWidth, y + halfHeight, z - halfDepth)
                    != Plane.PlaneSide.Back) continue;
            if (planes[i].testPoint(x + halfWidth, y - halfHeight, z + halfDepth)
                    != Plane.PlaneSide.Back) continue;
...
// the window coordinate system has it's y downwards. The
...
// float mouse_x, float mouse_y, Vector3 pos, Vector3 dir,
// Vector3 up) {
