...
import com.guidebee.math.collision.Ray;
import com.guidebee.math.geometry.Circle;
import com.guidebee.math.geometry.Plane;
...
public final class Intersector {
    ...
    /**
     * Returns whether the given point is inside the triangle. This
     * assumes that the point is on the plane of the triangle. No
     ...
     */
    public static boolean isPointInTriangle(Vector2 p, Vector2 a,
                                            Vector2 b, Vector2 c) {
        float px1 = p.x - a.x;
        float py1 = p.y - a.y;
        boolean side12 = (b.x - a.x) * py1
                - (b.y - a.y) * px1 > 0;
     ...
     * Returns true if the given point is inside the triangle.
     */
    public static boolean isPointInTriangle(float px, float py, float ax,
    ...
    /**
     * Determines on which side of the given line the point is. Returns -1
     * if the point is on the left side of the line, 0 if the
     * point is on the line and 1 if the point is on the right side of the
     * line. Left and right are relative to the lines direction
     * which is linePoint1 to linePoint2.
     */
    public static int pointLineSide(Vector2 linePoint1, Vector2 linePoint2,
                                    Vector2 point) {
    ...
    public static int pointLineSide(float linePoint1X, float linePoint1Y,
                                    float linePoint2X, float linePoint2Y, float pointX,
                                    float pointY) {
        return (int) Math.signum((linePoint2X - linePoint1X) * (pointY - linePoint1Y)
                - (linePoint2Y - linePoint1Y)
                * (pointX - linePoint1X));
    }
     ...
     */
    public static boolean isPointInPolygon(float[] polygon, int offset,
    ...
    /**
     * Returns the distance between the given line and point. Note the
     * specified line is not a line segment.
     */
    public static float distanceLinePoint(float startX, float startY, float endX,
                                          float endY, float pointX, float pointY) {
        float normalLength = (float) Math.sqrt((endX - startX) * (endX - startX)
                + (endY - startY) * (endY - startY));
        return Math.abs((pointX - startX) * (endY - startY) - (pointY - startY)
                * (endX - startX)) / normalLength;
    }

    /**
     * Returns the distance between the given segment and point.
     */
    public static float distanceSegmentPoint(float startX, float startY,
                                             float endX, float endY,
                                             float pointX, float pointY) {
        return nearestSegmentPoint(startX, startY, endX, endY, pointX,
                pointY, v2tmp).dst(pointX, pointY);
    }

    /**
     * Returns the distance between the given segment and point.
     */
    public static float distanceSegmentPoint(Vector2 start, Vector2 end,
                                             Vector2 point) {
     ...
     */
    public static Vector2 nearestSegmentPoint(Vector2 start, Vector2 end,
                                              Vector2 point, Vector2 nearest) {
        float length2 = start.dst2(end);
        if (length2 == 0) return nearest.set(start);
        float t = ((point.x - start.x) * (end.x - start.x)
                + (point.y - start.y) * (end.y - start.y)) / length2;
        if (t < 0) return nearest.set(start);
        if (t > 1) return nearest.set(end);
        return nearest.set(start.x + t * (end.x - start.x),
     ...
     * Returns a point on the segment nearest to the specified point.
     */
    public static Vector2 nearestSegmentPoint(float startX, float startY,
                                              float endX, float endY,
                                              float pointX, float pointY,
                                              Vector2 nearest) {
        ...
        float t = ((pointX - startX) * (endX - startX)
    ...
    /**
     * Checks whether the line segment and the circle intersect and returns
     * by how much and in what direction the line has to move
     ...
     * @param displacement The displacement vector set by the
     *                     method having unit length
     * @return The displacement or Float.POSITIVE_INFINITY if no
    ...
    /**
     * Intersect two 2D Rays and return the scalar parameter of the first
     * ray at the intersection point.
     ...
     * @return scalar parameter on the first ray describing the point where
     * the intersection happens. May be negative.
     * In case the rays are collinear, Float.POSITIVE_INFINITY will be returned.
     */
    public static float intersectRayRay(Vector2 start1, Vector2 direction1,
                                        Vector2 start2, Vector2 direction2) {
    ...
    /**
     * Intersects a {@link com.guidebee.math.collision.Ray} and a
     * {@link Plane}. The intersection point is stored in intersection in
     * case an intersection is
     * present.
     *
     * @param ray          The ray
    ...
    public static boolean intersectRayPlane(Ray ray, Plane plane,
        ...
        if (denom != 0) {
            float t = -(ray.origin.dot(plane.getNormal())
                    + plane.getD()) / denom;
            if (t < 0) return false;
     ...
     * distance from the first point to the plane. In case an
     * intersection happened, the return value is in the range [0,1].
     * The intersection point can be recovered by point1 + t *
     ...
     * @param plane
     */
    public static float intersectLinePlane(float x, float y, float z,
                                           float x2, float y2,
                                           float z2, Plane plane,
                                           Vector3 intersection) {
    ...
    /**
     * Intersect a {@link Ray} and a triangle, returning the intersection
     ...
     */
    public static boolean intersectRayTriangle(Ray ray, Vector3 t1,
                                               Vector3 t2, Vector3 t3,
                                               Vector3 intersection) {
        p.set(t1, t2, t3);
     ...
     * point in intersection.
     *
     * @param ray          The ray, the direction component must be
    ...
    public static boolean intersectRaySphere(Ray ray, Vector3 center,
        ...
        final float len = ray.direction.dot(center.x - ray.origin.x,
                center.y - ray.origin.y, center.z - ray.origin.z);
        if (len < 0.f) // behind the ray
            return false;
        final float dst2 = center.dst2(ray.origin.x + ray.direction.x * len,
                ray.origin.y + ray.direction.y * len, ray.origin.z
                        + ray.direction.z * len);
        final float r2 = radius * radius;
        if (dst2 > r2) return false;
        if (intersection != null)
            intersection.set(ray.direction).scl(len - (float) Math.sqrt(r2 - dst2))
                    .add(ray.origin);
        return true;
    }
     ...
     * @return Whether an intersection is present.
     */
    public static boolean intersectRayBounds(Ray ray, BoundingBox box,
                                             Vector3 intersection) {
        ...
        if (ray.origin.x <= box.min.x && ray.direction.x > 0) {
            ...
            if (t >= 0) {
                ...
                if (v2.y >= box.min.y && v2.y <= box.max.y && v2.z >= box.min.z
                        && v2.z <= box.max.z && (!hit || t < lowest)) {
                    hit = true;
                    lowest = t;
                }
            }
        ...
        if (ray.origin.y <= box.min.y && ray.direction.y > 0) {
            ...
            if (t >= 0) {
                v2.set(ray.direction).scl(t).add(ray.origin);
                if (v2.x >= box.min.x && v2.x <= box.max.x && v2.z >= box.min.z
                        && v2.z <= box.max.z && (!hit || t < lowest)) {
                    hit = true;
                    lowest = t;
                }
        ...
        if (ray.origin.z <= box.min.z && ray.direction.z > 0) {
            ...
            if (t >= 0) {
                v2.set(ray.direction).scl(t).add(ray.origin);
                if (v2.x >= box.min.x && v2.x <= box.max.x && v2.y >= box.min.y
                        && v2.y <= box.max.y && (!hit || t < lowest)) {
                    hit = true;
                    lowest = t;
                }
            }
     ...
     * @return Whether the ray and the bounding box intersect.
     */
    static public boolean intersectRayBoundsFast(Ray ray, Vector3 center,
                                                 Vector3 dimensions) {
    ...
    public static boolean intersectRayTriangles(Ray ray, float[] triangles,
        ...
        for (int i = 0; i < triangles.length - 6; i += 9) {
            boolean result = intersectRayTriangle(ray, tmp1.set(triangles[i],
                    ...
                    tmp2.set(triangles[i + 3], triangles[i + 4],
                            triangles[i + 5]),
     ...
     * @param indices      the indices, each successive 3 shorts index the
    ...
    public static boolean intersectRayTriangles(Ray ray, float[] vertices,
        ...
        for (int i = 0; i < indices.length; i += 3) {
            ...
            boolean result = intersectRayTriangle(ray, tmp1.set(vertices[i1],
     ...
     * @return Whether the ray and the triangles intersect.
     */
    public static boolean intersectRayTriangles(Ray ray, List<Vector3> triangles,
                                                Vector3 intersection) {
        float min_dist = Float.MAX_VALUE;
        boolean hit = false;
    ...
    /**
     * Intersects the two lines and returns the intersection
     * point in intersection.
     *
     ...
     */
    public static boolean intersectLines(Vector2 p1, Vector2 p2,
     ...
     * @return Whether the two lines intersect
     */
    public static boolean intersectLines(float x1, float y1, float x2,
     ...
     * sets the supplied {@code intersection} rectangle to the
     * area of overlap.
     *
     * @return Whether the rectangles intersect
     */
    static public boolean intersectRectangles(Rectangle rectangle1,
    ...
    /**
     * Intersects the two line segments and returns the intersection
     * point in intersection.
     *
    ...
    public static boolean intersectSegments(Vector2 p1, Vector2 p2, Vector2 p3,
        ...
        float x1 = p1.x, y1 = p1.y, x2 = p2.x, y2 = p2.y, x3 = p3.x,
                y3 = p3.y, x4 = p4.x, y4 = p4.y;
        ...
        if (intersection != null) intersection.set(x1 + (x2 - x1) * ua,
                y1 + (y2 - y1) * ua);
        return true;
    }
    ...
    /**
     * Check whether specified counter-clockwise wound convex polygons overlap.
     * If they do, optionally obtain a Minimum Translation Vector indicating the
     ...
     */
    public static boolean overlapConvexPolygons(float[] verts1, float[] verts2,
                                                MinimumTranslationVector mtv) {
        return overlapConvexPolygons(verts1, 0, verts1.length, verts2, 0,
                verts2.length, mtv);
    }

    /**
     * Check whether polygons defined by the given counter-clockwise wound
     * vertex arrays overlap. If they do, optionally obtain a Minimum Translation
     * Vector indicating the minimum magnitude vector required to push the
     ...
     * @return Whether polygons overlap.
     */
    public static boolean overlapConvexPolygons(float[] verts1, int offset1,
                                                int count1, float[] verts2,
     ...
     * instance. Depending on where the triangle is
     * relative to the plane, the result can be:
     * <p/>
     * <ul>
     * <li>Triangle is fully in front/behind: {@link SplitTriangle#front} or
     * {@link SplitTriangle#back} will contain the original
     * triangle, {@link SplitTriangle#total} will be one.</li>
     * <li>Triangle has two vertices in front, one behind: {@link SplitTriangle#front}
     * contains 2 triangles,
     * {@link SplitTriangle#back} contains 1 triangles, {@link SplitTriangle#total}
     * will be 3.</li>
     * <li>Triangle has one vertex in front, two behind: {@link SplitTriangle#front}
     * contains 1 triangle,
     * {@link SplitTriangle#back} contains 2 triangles, {@link SplitTriangle#total}
     * will be 3.</li>
     * </ul>
     * <p/>
     * The input triangle should have the form: x, y, z, x2, y2, z2, x3, y3, y3.
     * One can add additional attributes per vertex which
     * will be interpolated if split, such as texture coordinates or normals.
     * Note that these additional attributes won't be
     ...
     */
    public static void splitTriangle(float[] triangle, Plane plane,
                                     SplitTriangle split) {
        int stride = triangle.length / 3;
        boolean r1 = plane.testPoint(triangle[0], triangle[1],
                triangle[2]) == Plane.PlaneSide.Back;
        boolean r2 = plane.testPoint(triangle[0 + stride],
                triangle[1 + stride], triangle[2 + stride]) == Plane.PlaneSide.Back;
        boolean r3 = plane.testPoint(triangle[0 + stride * 2],
                triangle[1 + stride * 2], triangle[2 + stride * 2]) == Plane.PlaneSide.Back;
    ...
    private static void splitEdge(float[] vertices, int s, int e, int stride,
                                  Plane plane, float[] split, int offset) {
        float t = Intersector.intersectLinePlane(vertices[s], vertices[s + 1],
                vertices[s + 2], vertices[e], vertices[e + 1],
    ...
    public static void main(String[] args) {
        ...
        // Note, in stage the bottom and left sides are inclusive while the
        // right and top sides are exclusive.
    }
    ...
    public static class SplitTriangle {
        ...
        public String toString() {
            return "SplitTriangle [front=" + Arrays.toString(front) + ", back="
                    + Arrays.toString(back) + ", numFront=" + numFront
