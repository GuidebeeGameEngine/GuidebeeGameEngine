...
import com.guidebee.utils.collections.BooleanArray;
import com.guidebee.utils.collections.FloatArray;
import com.guidebee.utils.collections.IntArray;
import com.guidebee.utils.collections.ShortArray;
...
public class DelaunayTriangulator {
     ...
     *
     * @param points x,y pairs describing points. Duplicate points will result
     *               in undefined behavior.
     * @param sorted If false, the points will be sorted by the x coordinate,
     *               which is required by the triangulation algorithm. If
     *               sorting is done the input array is not modified, the
     *               returned indices are for the input array, and count*2
     *               additional working memory is needed.
     * @return triples of indices into the points that describe the triangles
     * in clockwise order. Note the returned array is reused
     * for later calls to the same method.
     */
    public ShortArray computeTriangles(float[] points, int offset, int count,
                                       boolean sorted) {
        ...
        for (int pointIndex = offset; pointIndex < end; pointIndex += 2) {
            ...
            for (int triangleIndex = triangles.size - 1; triangleIndex >= 0;
                 triangleIndex -= 3) {
        ...
        if (!sorted) {
            ...
            for (int i = 0, n = triangles.size; i < n; i++)
                trianglesArray[i]
                        = (short) (originalIndicesArray[trianglesArray[i] / 2] * 2);
        }
     ...
     * points x1,y1, x2,y2, x3,y3. Returns COMPLETE if xp
     * is to the right of the entire circumcircle. Otherwise returns INCOMPLETE
     * . Note: a point on the circumcircle edge is
     * considered inside.
     */
    private int circumCircle(float xp, float yp, float x1, float y1,
    ...
    private int quicksortPartition(final float[] values, int lower,
                                   int upper, short[] originalIndices) {
    ...
    /**
     * Removes all triangles with a centroid outside the specified hull,
     * which may be concave. Note some triangulations may have
     * triangles whose centroid is inside the hull but a portion is outside.
     */
    public void trim(ShortArray triangles, float[] points, float[] hull,
        ...
        for (int i = triangles.size - 1; i >= 0; i -= 3) {
            ...
            GeometryUtils.triangleCentroid(points[p1], points[p1 + 1],
                    points[p2], points[p2 + 1], points[p3], points[p3 + 1],
                    centroid);
            if (!Intersector.isPointInPolygon(hull, offset, count,
