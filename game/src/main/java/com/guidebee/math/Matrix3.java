...
/**
 * A 3x3 <a href="http://en.wikipedia.org/wiki/Row-major_order#Column-major_order">
 * column major</a> matrix; useful for 2D
 * transforms.
 *
 * @author mzechner
 */
public class Matrix3 implements Serializable {
     ...
     * at least 9 elements; the first 9 will be copied.
     *
     ...
     *               column major</a> order. (The float array is
     *               not modified.)
     */
     ...
     */
    public float det() {
        return val[M00] * val[M11] * val[M22] + val[M01] * val[M12]
                * val[M20] + val[M02] * val[M10] * val[M21] - val[M00]
                * val[M12] * val[M21] - val[M01] * val[M10]
                * val[M22] - val[M02] * val[M11] * val[M20];
    }
     ...
     *
     * @return This matrix for the purpose of chaining operations.
     * @throws com.guidebee.game.GameEngineRuntimeException if the matrix is singular (not invertible)
     */
    public Matrix3 inv() {
        float det = det();
        if (det == 0) throw new GameEngineRuntimeException("Can't invert a singular matrix");
     ...
     *
     * @param mat The matrix whose top left corner will be copied.
    ...
    /**
     * Sets the matrix to the given matrix as a float array. The float
     * array must have at least 9 elements; the first 9 will be
     * copied.
     *
     ...
     * The other columns are untouched.
     *
     * @param vector The translation vector. (The z-component of the
     *               vector is ignored because this is a 3x3 matrix)
     * @return This matrix for the purpose of chaining.
     */
    ...
    /**
     * Scale the matrix in the both the x and y components by
     * the scalar value.
     *
     * @param scale The single value that will be used to scale
     *              both the x and y components.
     * @return This matrix for the purpose of chaining methods
     * together.
     */
    ...
    /**
     * Scale this matrix using the x and y components of the
     * vector but leave the rest of the matrix alone.
     *
     ...
     * </pre>
     *
     * @param mata The float array representing the first matrix.
     *             Must have at least 9 elements.
     * @param matb The float array representing the second matrix.
     *             Must have at least 9 elements.
     */
