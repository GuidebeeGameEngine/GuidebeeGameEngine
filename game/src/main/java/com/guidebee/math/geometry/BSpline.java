...
public class BSpline<T extends Vector<T>> implements Path<T> {
    ...
    /**
     * Calculates the cubic b-spline value for the given span (i) at the
     * given position (u).
     *
     * @param out        The Vector to set to the result.
     ...
     * @return The value of out
     */
    public static <T extends Vector<T>> T cubic(final T out, final int i,
                                                final float u, final T[] points,
                                                final boolean continuous,
                                                final T tmp) {
        ...
        if (continuous || i > 0) out.add(tmp.set(points[(n + i - 1) % n])
                .scl(dt * dt * dt * d6));
        if (continuous || i < (n - 1))
            out.add(tmp.set(points[(i + 1) % n])
    ...
    /**
     * Calculates the cubic b-spline derivative for the given span (i)
     ...
     * @return The value of out
     */
    public static <T extends Vector<T>> T calculate(final T out,
                                                    final float t,
                                                    final T[] points,
                                                    final int degree,
                                                    final boolean continuous,
     ...
     * @return The value of out
     */
    public static <T extends Vector<T>> T derivative(final T out,
                                                     final float t,
                                                     final T[] points,
                                                     final int degree,
                                                     final boolean continuous,
    ...
    /**
     * Calculates the n-degree b-spline value for the given span (i) at the
     * given position (u).
     *
     * @param out        The Vector to set to the result.
     * @param i          The span (0<=i<spanCount) spanCount = continuous
    ...
    /**
     * Calculates the n-degree b-spline derivative for the given span (i)
     * at the given position (u).
     *
     * @param out        The Vector to set to the result.
     * @param i          The span (0<=i<spanCount) spanCount = continuous ?
    ...
    public BSpline(final T[] controlPoints, final int degree,
                   final boolean continuous) {
    ...
    public BSpline set(final T[] controlPoints, final int degree,
        ...
        this.spanCount = continuous ? controlPoints.length
        ...
        for (int i = 0; i < spanCount; i++)
            knots.add(calculate(controlPoints[0].cpy(), continuous ? i
                            : (int) (i + 0.5f * degree), 0f, controlPoints, degree,
                    continuous, tmp));
        return this;
    }
    ...
    public T valueAt(final T out, final int span, final float u) {
        return calculate(out, continuous ? span
                : (span + (int) (degree * 0.5f)), u, controlPoints,
    ...
    public T derivativeAt(final T out, final int span, final float u) {
        return derivative(out, continuous ? span
                : (span + (int) (degree * 0.5f)), u, controlPoints,
                degree, continuous, tmp);
    }
    ...
    /**
     * @return The span closest to the specified value,
     * restricting to the specified spans.
     */
