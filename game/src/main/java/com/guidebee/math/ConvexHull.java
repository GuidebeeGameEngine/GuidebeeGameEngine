...
public class ConvexHull {
     ...
     *  Note: the last point in the returned list is the
     * same as the first one. */
    /**
     * Returns the convex hull polygon for the given point cloud.
     *
     * @param points x,y pairs describing points. Duplicate points will
     *               result in undefined behavior.
     * @param sorted If false, the points will be sorted by the x coordinate
     *               then the y coordinate, which is required by the convex
     *               hull algorithm. If sorting is done the input array is
     *               not modified and count additional working memory is needed.
     * @return pairs of coordinates that describe the convex hull polygon in
     * counterclockwise order. Note the returned array is
    ...
    /**
     * Computes a hull the same as {@link #computePolygon(float[], int, int, boolean)}
    ...
    public IntArray computeIndices(float[] points, int offset, int count,
        ...
        // Upper hull.
        for (int i = end - 4, index = i / 2,
                     t = hull.size + 2; i >= offset; i -= 2, index--) {
            float x = points[i];
            float y = points[i + 1];
            while (hull.size >= t && ccw(x, y) <= 0) {
                hull.size -= 2;
    ...
    /**
     * Returns > 0 if the points are a counterclockwise turn, < 0
     * if clockwise, and 0 if colinear.
     */
    ...
    private void sortWithIndices(float[] values, int count, boolean yDown) {
        ...
        while (stack.size > 0) {
            ...
            lower = stack.pop();
            if (upper <= lower) continue;
            int i = quicksortPartitionWithIndices(values, lower, upper,
                    yDown, originalIndicesArray);
            if (i - lower > upper - i) {
                stack.add(lower);
                stack.add(i - 2);
            }
