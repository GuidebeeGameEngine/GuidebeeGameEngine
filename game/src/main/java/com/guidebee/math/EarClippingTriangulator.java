 ...
 * <ul>
 * <li><a href="http://cgm.cs.mcgill.ca/~godfried/teaching/cg-projects/97/Ian/algorithm2.html">
 * http://cgm.cs.mcgill.ca/~godfried/
 * teaching/cg-projects/97/Ian/algorithm2.html</a></li>
 * <li><a
 * href="http://www.geometrictools.com/Documentation/TriangulationByEarClipping.pdf">
 * http://www.geometrictools.com/Documentation
 * /TriangulationByEarClipping.pdf</a></li>
 * </ul>
 * If the input polygon is not simple (self-intersects), there will be output but it
 * is of unspecified quality (garbage in,
 * garbage out).
 *
...
public class EarClippingTriangulator {
     ...
     * triangle vertices.
     *
     * @param vertices pairs describing vertices of the polygon, in either
     *                 clockwise or counterclockwise order.
     * @return triples of triangle indices in clockwise order. Note the
     * returned array is reused for later calls to the same
     * method.
     */
    public ShortArray computeTriangles(float[] vertices, int offset,
                                       int count) {
    ...
    private int classifyVertex(int index) {
        ...
        return computeSpannedAreaSign(vertices[previous], vertices[previous + 1],
    ...
    private int findEarTip() {
        ...
        // Note that the input was not necessarily degenerate, but we could
        // have made it so by clipping some valid ears.

        // Idea taken from Martin Held, "FIST: Fast industrial-strength
        // triangulation of polygons", Algorithmica (1998),
    ...
    private boolean isEarTip(int earTipIndex) {
        ...
        for (int i = nextIndex(nextIndex);
             i != previousIndex; i = nextIndex(i)) {
            // Concave vertices can obviously be inside the candidate ear,
            // but so can tangential vertices
            // if they coincide with one of the triangle's vertices.
            if (vertexTypes[i] != CONVEX) {
                int v = indices[i] * 2;
                float vx = vertices[v];
                float vy = vertices[v + 1];
                // Because the polygon has clockwise winding order, the
                // area sign will be positive if the point is strictly inside.
                // It will be 0 on the edge, which we want to include as well.
                // note: check the edge defined by p1->p3 first since this
                // fails _far_ more then the other 2 checks.
    ...
    static private int computeSpannedAreaSign(float p1x, float p1y, float p2x,
