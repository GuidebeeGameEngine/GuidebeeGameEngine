 ...
 * column major</a> 4 by 4 matrix. Like
 * the {@link Vector3} class it allows the chaining of methods by returning
 * a reference to itself. For example:
 * <p/>
 * <pre>
...
public class Matrix4 implements Serializable {
     ...
     * of the angle when rotated on the Y and/or Z axis. On
     ...
     * multiplication this value is multiplied with the
     * source X component and added to the target Y component.
     */
    public static final int M10 = 1;
    /**
     * YY: Typically the unrotated Y component for scaling, also the cosine of
     * the angle when rotated on the X and/or Z axis. On
     ...
     * the angle when rotated on the X and/or Y axis. On
     ...
     *               href="http://en.wikipedia.org/wiki/Row-major_order">column major</a>
     ...
     * have at least 16 elements; the first 16 will be
     * copied.
     *
     * @param values The matrix, in float form, that is to be copied.
     *               Remember that this matrix is in <a
     *               href="http://en.wikipedia.org/wiki/Row-major_order">
    ...
    /**
     * Sets the matrix to a rotation matrix representing the translation
     * and quaternion.
     *
     * @param translationX The X component of the translation that is to
     *                     be used to set this matrix.
     * @param translationY The Y component of the translation that is to
     *                     be used to set this matrix.
     * @param translationZ The Z component of the translation that is to
     *                     be used to set this matrix.
     * @param quaternionX  The X component of the quaternion that is to
     *                     be used to set this matrix.
     * @param quaternionY  The Y component of the quaternion that is to
     *                     be used to set this matrix.
     * @param quaternionZ  The Z component of the quaternion that is to
     *                     be used to set this matrix.
     * @param quaternionW  The W component of the quaternion that is to
     *                     be used to set this matrix.
     * @return This matrix for the purpose of chaining methods together.
     */
    ...
    public Matrix4 set(float translationX, float translationY, float translationZ,
                       ...
                       float quaternionZ, float quaternionW, float scaleX,
    ...
    /**
     * Sets the four columns of the matrix which correspond to the x-, y-
     * and z-axis of the vector space this matrix creates as
     * well as the 4th column representing the translation of any point
    ...
    /**
     * Postmultiplies this matrix with the given matrix, storing the
    ...
    /**
     * Premultiplies this matrix with the given matrix, storing the
     * result in this matrix. For example:
     * <p/>
     * <pre>
    ...
    public Matrix4 inv() {
        float l_det = val[M30] * val[M21] * val[M12] * val[M03]
                ...
                * val[M02] * val[M23] - val[M30] * val[M01] * val[M12]
                * val[M23] + val[M00] * val[M31] * val[M12] * val[M23] + val[M10]
                * val[M01] * val[M32] * val[M23] - val[M00] * val[M11]
                * val[M32] * val[M23] - val[M20] * val[M11] * val[M02] * val[M33]
                + val[M10] * val[M21] * val[M02] * val[M33] + val[M20]
                * val[M01] * val[M12] * val[M33] - val[M00] * val[M21] * val[M12]
    ...
    public float det() {
        return val[M30] * val[M21] * val[M12] * val[M03] - val[M20] * val[M31]
                * val[M12] * val[M03] - val[M30] * val[M11]
                * val[M22] * val[M03] + val[M10] * val[M31] * val[M22] * val[M03]
                + val[M20] * val[M11] * val[M32] * val[M03] - val[M10]
                * val[M21] * val[M32] * val[M03] - val[M30] * val[M21]
                * val[M02] * val[M13] + val[M20] * val[M31] * val[M02] * val[M13]
                + val[M30] * val[M01] * val[M22] * val[M13] - val[M00]
                * val[M31] * val[M22] * val[M13] - val[M20] * val[M01]
                * val[M32]
                * val[M13] + val[M00] * val[M21] * val[M32] * val[M13]
                + val[M30] * val[M11] * val[M02] * val[M23] - val[M10] * val[M31]
                * val[M02] * val[M23] - val[M30] * val[M01] * val[M12]
    ...
    /**
     * Sets the matrix to a projection matrix with a near- and far plane,
     * a field of view in degrees and an aspect ratio.
     *
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Matrix4 setToProjection(float near, float far, float fov,
                                   float aspectRatio) {
        idt();
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Matrix4 setToOrtho2D(float x, float y, float width,
     ...
     * (http://www.opengl.org/sdk/docs/man/xhtml/glOrtho.xml) following
     ...
     * overwriting it with an identity and then setting the
     * translation vector in the 4th column and the scaling vector
     * in the diagonal.
     *
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Matrix4 setToRotation(float axisX, float axisY,
     ...
     * @return This matrix for the purpose of chaining methods together.
     */
    public Matrix4 setToRotationRad(float axisX, float axisY,
                                    float axisZ, float radians) {
        if (radians == 0) {
            idt();
            return this;
        }
    ...
    /**
     * Sets the matrix to a look at matrix with a direction and an
     * up vector. Multiply with a translation matrix to get a camera
     * model view matrix.
     *
    ...
    /**
     * Sets this matrix to a look at matrix with the given position,
    ...
    /**
     * Averages the given transforms with the given weights and stores
    ...
    /**
     * Sets this matrix to the given 3x3 matrix. The third column of
     * this matrix is set to (0,0,1,0).
     *
     ...
     * @param normalizeAxes True to normalize the axes, necessary
     *                      when the matrix might also include scaling.
    ...
    public float getScaleX() {
        return (MathUtils.isZero(val[Matrix4.M01])
                && MathUtils.isZero(val[Matrix4.M02])) ? Math.abs(val[Matrix4.M00])
    ...
    public float getScaleY() {
        return (MathUtils.isZero(val[Matrix4.M10])
                && MathUtils.isZero(val[Matrix4.M12])) ? Math.abs(val[Matrix4.M11])
    ...
    public float getScaleZ() {
        return (MathUtils.isZero(val[Matrix4.M20])
                && MathUtils.isZero(val[Matrix4.M21])) ? Math.abs(val[Matrix4.M22])
                : (float) Math.sqrt(getScaleZSquared());
    }

    /**
     * @param scale The vector which will receive
     *              the (non-negative) scale components on each axis.
    ...
    /**
     * Multiplies the matrix mata with matrix matb, storing the result
     * in mata. The arrays are assumed to hold 4x4 column major
     * matrices as you can get from {@link Matrix4#val}. This is the
     * same as {@link Matrix4#mul(Matrix4)}.
     *
	...
	*/
    ...
    /**
     * Multiplies the vector with the given matrix. The matrix array
     * is assumed to hold a 4x4 column major matrix as you can get
     * from {@link Matrix4#val}. The vector array is assumed to hold
     * a 3-component vector, with x being the first element, y being
     * the second and z being the last component. The result is stored
     * in the vector array. This is the same as
	...
	*/
    ...
    /**
     * Multiplies the vectors with the given matrix. The matrix array
     * is assumed to hold a 4x4 column major matrix as you can get
     * from {@link Matrix4#val}. The vectors array is assumed to hold
     * 3-component vectors. Offset specifies the offset into the
     * array where the x-component of the first vector is located.
     * The numVecs parameter specifies the number of vectors stored in
     * the vectors array. The stride parameter specifies the number
     * of floats between subsequent vectors and must be >= 3. This is
     ...
     */
    public static native void mulVec(float[] mat, float[] vecs,
                                     int offset, int numVecs, int stride) /*-{
    }-*/; /*
	...
	*/
    ...
    /**
     * Multiplies the vector with the given matrix, performing a
     * division by w. The matrix array is assumed to hold a 4x4 column
     * major matrix as you can get from {@link Matrix4#val}. The
     * vector array is assumed to hold a 3-component vector, with x being
     * the first element, y being the second and z being the last
     * component. The result is stored in the vector array. This is the
	...
	*/
    ...
    /**
     * Multiplies the vectors with the given matrix, , performing a
     * division by w. The matrix array is assumed to hold a 4x4 column
     * major matrix as you can get from {@link Matrix4#val}. The
     * vectors array is assumed to hold 3-component vectors. Offset
     * specifies the offset into the array where the x-component of
     * the first vector is located. The numVecs parameter specifies
     * the number of vectors stored in the vectors array. The stride
     * parameter specifies the number of floats between subsequent
     * vectors and must be >= 3. This is the same as {@link Vector3#prj(Matrix4)}
     ...
     * @param stride  the stride between vectors in floats
     */
    public static native void prj(float[] mat, float[] vecs, int offset,
	...
	*/
    ...
    /**
     * Multiplies the vector with the top most 3x3 sub-matrix of the
     * given matrix. The matrix array is assumed to hold a 4x4 column
	...
	*/
    ...
    /**
     * Multiplies the vectors with the top most 3x3 sub-matrix of the
     * given matrix. The matrix array is assumed to hold a 4x4
     * column major matrix as you can get from {@link Matrix4#val}.
     * The vectors array is assumed to hold 3-component vectors.
     * Offset specifies the offset into the array where the x-component
     * of the first vector is located. The numVecs parameter
     * specifies the number of vectors stored in the vectors array.
     * The stride parameter specifies the number of floats between
     ...
     * @param stride  the stride between vectors in floats
     */
    public static native void rot(float[] mat, float[] vecs,
                                  int offset, int numVecs, int stride) /*-{
    }-*/; /*
		float* vecPtr = vecs + offset;
		for(int i = 0; i < numVecs; i++) {
	...
	*/
    ...
    /**
     * Computes the inverse of the given matrix. The matrix array
     * is assumed to hold a 4x4 column major matrix as you can get from
     * {@link Matrix4#val}.
     *
	...
	*/
    ...
    /**
     * Computes the determinante of the given matrix. The matrix array
     * is assumed to hold a 4x4 column major matrix as you can get
